"""SHS-RSSM on a multi-object switching scene: the consolidated, working demo.

Scene: 32x32 RGB, four colored shapes (disk/square/spinning-triangle/diamond); three present
from t=0, the diamond appears at the midpoint (composition change). All shapes orbit at their
own speed under a shared 3-state sticky switching mode; the triangle also spins.

Model: the real SHSRSSM (continuous latent, K-component switching linear-Gaussian transition,
sticky HDP-HMM) wrapped in the strongest encoder/decoder that trains reliably on CPU here
(RGB conv enc/dec with GroupNorm). RECURRENT STICKINESS is ON: the regime persistence is
h-dependent via Polya-Gamma augmentation (shs_recurrent=True), instead of a single global
stickiness kappa.

Outputs three diagnostics:
  fig5  latent regime clustering (sticky-HDP on the learned latent)
  fig6  reconstruction + open-loop imagination across the composition change
  fig7  true-vs-inferred switching dynamics (timelines, transition matrix, recovered omega)

Usage:  python shs_demo.py [ITERS]      (RESUME=1 continues from checkpoint)
"""
import sys, os, time
import numpy as np
import torch
...
if not hasattr(torch, "softplus"):
    torch.softplus = F.softplus          # repo expects torch.softplus; shim for newer torch

_HERE = os.path.dirname(os.path.abspath(__file__))   # this file lives in the repo root
sys.path.insert(0, _HERE)                            # so shs_rssm/ and networks.py import
_OUT = os.environ.get("SHS_OUT", _HERE)              # where figures are written
...
H = W = 32
STOCH, DETER, EMBED, K = 24, 96, 192, 6      # larger latent + embedding for the color scene
FEAT = STOCH + DETER
CH = 3                                        # RGB
SHS_RECURRENT = True                          # h-dependent (Polya-Gamma) sticky persistence

# ============================================================ data: multi-object video
# ---- shared 3-state switching motion mode (sticky HDP-HMM ground truth) ----
ANGS = [0.45, -0.75, 1.10]                   # per-step orbital angular velocity per mode
stay, Ktrue = 0.97, 3
P = np.full((Ktrue, Ktrue), (1 - stay) / (Ktrue - 1)); np.fill_diagonal(P, stay)

# ---- per-object geometry / dynamics (4 shapes; #3 appears mid-episode) ----
N_OBJ      = 4
T          = 48
T_APPEAR   = T // 2                          # the diamond pops in here
SHAPES     = ["disk", "square", "triangle", "diamond"]
RADII      = [0.58, 0.62, 0.26, 0.46]        # orbit radius of each object's centre
SPEED_MULT = [0.70, 1.25, 0.50, 1.00]        # different orbital speeds (x the shared mode)
PHASE0     = [0.30, 2.45, 4.00, 5.25]        # spread starting angles around the circle
SIZE       = [4.0, 3.6, 5.6, 4.6]            # shape radius in pixels
SPIN_OBJ   = 2                               # the triangle spins about itself
SPIN_RATE  = 0.55                            # rad / step

B = 18
pos  = np.zeros((B, T, N_OBJ, 2))            # object centres in [-1,1]^2
spin = np.zeros((B, T))                      # triangle orientation
S    = np.zeros((B, T), dtype=int)           # shared switching mode (ground truth)
for b in range(B):
    s = np.random.randint(Ktrue)
    th = np.array(PHASE0) + np.random.uniform(-0.3, 0.3, N_OBJ)
    p = np.stack([np.array(RADII) * np.cos(th), np.array(RADII) * np.sin(th)], axis=1)  # (N,2)
    ...
    for t in range(T):
        if t > 0:
            ...
            for i in range(N_OBJ):
                ...
                p[i] = p[i] / (np.linalg.norm(p[i]) + 1e-8) * RADII[i]   # stay on the circle
            sp += SPIN_RATE
        S[b, t] = s; pos[b, t] = p; spin[b, t] = sp

# ---- visibility: objects 0,1,2 always on; object 3 fades in over a few frames at T_APPEAR
vis = np.ones((B, T, N_OBJ))
ramp = np.clip((np.arange(T) - T_APPEAR) / 3.0 + 1.0, 0.0, 1.0)   # 0 before, ->1 after
vis[:, :, 3] = ramp[None, :]

# ---- soft-edged shape rasteriser (each returns (B,T,H,W) in [0,1]); combine by max ----
...
def shape_field(kind, pos2d, size, sharp=0.7, angle=None):
    ...
    if kind == "disk":
        ...
        rx, ry = ca * dx - sa * dy, sa * dx + ca * dy        # rotate into shape frame
        apo = size * 0.5
        d1 = -ry - apo                                       # outward normals at 270/150/30 deg
...
COLORS = np.array([[0.92, 0.16, 0.16],     # disk   -> red
                   [0.18, 0.78, 0.22],     # square -> green
                   [0.20, 0.42, 0.96],     # triangle (spinning) -> blue
                   [0.97, 0.82, 0.10]])    # diamond (appears mid) -> yellow
frame = np.zeros((B, T, H, W, 3))           # paint shapes (alpha-composited) over black
for i, kind in enumerate(SHAPES):
    ang = spin if i == SPIN_OBJ else None
    a = (shape_field(kind, pos[:, :, i, :], SIZE[i], sharp=0.5, angle=ang)
         * vis[:, :, i][..., None, None])[..., None]          # (B,T,H,W,1) crisp alpha
    frame = a * COLORS[i][None, None, None, None, :] + (1 - a) * frame
frames = torch.tensor(frame.transpose(0, 1, 4, 2, 3), dtype=torch.float32)   # (B,T,3,H,W)
...
class Enc(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(
            nn.Conv2d(CH, 32, 3, 2, 1), nn.GroupNorm(8, 32), nn.SiLU(),    # 32 -> 16
            nn.Conv2d(32, 64, 3, 2, 1), nn.GroupNorm(8, 64), nn.SiLU(),    # 16 -> 8
            nn.Conv2d(64, 128, 3, 2, 1), nn.GroupNorm(8, 128), nn.SiLU())  # 8 -> 4
...
class Dec(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(
            nn.ConvTranspose2d(128, 64, 4, 2, 1), nn.GroupNorm(8, 64), nn.SiLU(),  # 4 -> 8
            nn.ConvTranspose2d(64, 32, 4, 2, 1), nn.GroupNorm(8, 32), nn.SiLU(),   # 8 -> 16
            nn.ConvTranspose2d(32, CH, 4, 2, 1))                                   # 16 -> 32
    def forward(self, f):
...
if ITERS == 0:
    ...
    if os.environ.get("RESUME") and os.path.exists(CKPT):     # continue from a prior run
        sd = torch.load(CKPT)
        enc.load_state_dict(sd['enc']); dec.load_state_dict(sd['dec']); rssm.load_state_dict(sd['rssm'])
...
def fit_regimes_posthoc(z, K0=8, iters_burn=18, iters_prune=12):
    """Cluster the learned latent dynamics with the sticky-HDP regime model (VB + ELBO-gated
    birth/merge/delete). proj_dim=0 -> the regimes must explain z_{t-1}->z_t themselves, so
    they cluster the latent motion. z: (B,T,L)."""
    B, T, L = z.shape
    ...
    with torch.no_grad():                       # diverse init: seed distinct rotation blocks
        for k in range(K0):
            M0 = torch.eye(L)
            if L >= 2:
...
def relabel_to_true(gamma, S, ktrue):
    """Map each inferred regime to its best-matching true mode; aggregate responsibilities
    into (B,T,ktrue) so colours/occupancy align with the truth. Robust to any K."""
...
def plot_true_vs_inferred_dynamics(S, gamma, pos, angs, mult, Ptrue, path, n_ep=5):
    """The plot that actually shows what the SHS-RSSM is responsible for: recovery of the
    SWITCHING DYNAMICS. Three panels, all 'true vs inferred':
      (1) per-episode regime timelines (true mode strip above its inferred strip);
      (2) the HMM transition matrix (true sticky P vs the inferred one);
      (3) the per-step angular velocity each regime captured vs the true mode value."""
...
# how linearly does the latent encode the visible scene state? (positions of the 3
# always-present objects + the spinning triangle's orientation as cos/sin)
scene = np.concatenate([pos[:, :, :3, :].reshape(B, T, 6),
                        np.cos(spin)[..., None], np.sin(spin)[..., None]], axis=-1)  # (B,T,8)
...
# fit the sticky-HDP regime model on the LEARNED latent dynamics
...
# reconstruction + open-loop imagination for one episode (context spans the composition change)
with torch.no_grad():
    ep, ctx = 0, T_APPEAR + 6                 # condition past the moment the diamond appears
    x = frames[ep:ep + 1]
    emb = encode_all(x)
