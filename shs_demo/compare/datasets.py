...
def load_mocap6(standardize=True, path=None):
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"{path} is a data artifact not shipped with the code (its "
            "generator is a python2/bnpy provenance script). Copy it from a "
            "previous deployment, e.g.: scp <old-cluster>:.../AIME/shs_demo/"
            "mocap6/dataset.npz shs_demo/mocap6/")
...
LOADERS = dict(fhn=load_fhn, nascar=load_nascar, toyark13=load_toyark13, mocap6=load_mocap6)
