...
try:
    ...
    import Image
from utilities import AverageMeter, ResidualBlock, LinearResidual, ResidualBlock_deconv, UnetEncoder, DecoderCell, SqueezeUnetEncoder
...
class GMMVAE(nn.Module):
    ...
    def __init__(self, number_of_mixtures, nchannel, z_dim, w_dim, hidden_dim,  device, img_width, batch_size, max_filters=512, num_layers=4, small_conv=False ,dropout=0.1):
        super(GMMVAE, self).__init__()
        cfg = {'PicaNet': "GGGLLL",
            'Size': [img_width//8, img_width//8, img_width//8, 64, 64, 128, 128],
            #'Channel': [w_dim, 1024, 512, 512, 256, 128, 64],
            'Channel': [w_dim, 1024, 512, 384, 256, 128, 96],
            'loss_ratio': [0.5, 0.5, 0.5, 0.5, 0.8, 0.8, 1]}
        ...
        #self.encoder_w   = UnetEncoder(w_dim)
        self.encoder_w       = SqueezeUnetEncoder(w_dim)
        self.decoder_w       = nn.ModuleList()
        self.cfg = cfg
        for i in range(6):
            ...
            self.decoder_w.append(
                DecoderCell(size=cfg['Size'][i],
                            ...
                            mode=cfg['PicaNet'][i]))
        self.decoder_w.append(DecoderCell(size=cfg['Size'][6],
                                        in_channel=cfg['Channel'][6],
                                        out_channel=1,
                                        mode='C'))
        ...
        #self.conv1d0_w   = nn.Conv1d(w_dim*(img_width//16)**2, w_dim, 1)
        self.conv1d0_w   = nn.Conv1d((16*7)**2, w_dim, 1)
        self.bn1d0_w     = nn.BatchNorm1d(w_dim)

        # mean of w
        self.fc1_w        = nn.Linear(w_dim, w_dim, bias=False)
        self.bn1d1_w      = nn.BatchNorm1d(w_dim)

        # logvar_w
        self.fc2_w        = nn.Linear(w_dim, w_dim, bias=False)
        self.bn1d2_w      = nn.BatchNorm1d(w_dim)
        self.drop         = nn.Dropout(dropout)


        ########################
        #number of mixtures (c parameter) gets the input from feedforward layers of w and convolutional layers of z
        self.fc0         = nn.Linear(hidden_dim+w_dim, hidden_dim)
        self.bn1d_0      = nn.BatchNorm1d(hidden_dim)
        self.fc1         = nn.Linear(hidden_dim, number_of_mixtures)
        self.bn1d_1      = nn.BatchNorm1d(number_of_mixtures)
        ########################
        #(GENERATOR)
    ...
    def GMM_encoder(self, data):
        ...
        hw                = data
        self.en_out       = self.encoder_w(hw)
        #print(f"last layer of an encoder :{self.en_out[-1].size()}")
        #print(f"first layer if the encoder :{self.en_out[0].size()}")
        dec = None
        self.pred = []
        for i in range(7):
            # print(En_out[5 - i].size())
            dec, _pred = self.decoder_w[i](self.en_out[6 - i], dec)
            self.pred.append(_pred)
        
        hw                = self.pool(self.en_out[-1])

        hw                = self.flatten_raw_img(hw)
        #hw                = self.drop(hw)
        #print(f"last layer of encoder after pooling :{hw.size()}")
        hw                = hw.unsqueeze(-1)
        hlayer_w          = F.relu(self.bn1d0_w(self.conv1d0_w(hw).squeeze()))
        #print(hlayer_w.size())
        #mean of W
        mu_w              = F.relu(self.bn1d1_w(self.fc1_w(hlayer_w)))
        #variance of Q(w|x) distribution

        #log(variance of W)
        logvar_w          = F.relu(self.bn1d2_w(self.fc2_w(hlayer_w)))
        ...
        hc               = zw
        hc               = F.relu(self.bn1d_0(self.fc0(hc)))
        Pc_wz            = F.relu(self.bn1d_1(self.fc1(hc)))
...
class InfGaussMMVAE(GMMVAE,BetaSample):
    ...
    def __init__(self, hyperParams, K, nchannel, z_dim, w_dim, hidden_dim, device, img_width, batch_size, include_elbo2=True):
        global local_device
        local_device = device
        super(InfGaussMMVAE, self).__init__(K, nchannel, z_dim, w_dim, hidden_dim,  device, img_width, batch_size, max_filters=512, num_layers=4, small_conv=False, dropout=0.1)
    ...
    def get_ELBO(self, X):
        ...
        #7) loss function related to the salient network
        #source:https://github.com/Huiyu-Li/CECT-Segmentation/blob/master/2020_PUB-SalNet/network.py#L51
        temp     = self.pred[-1].detach().cpu().numpy()
        ...
        elbo7    =  F.binary_cross_entropy(self.pred[-1], pred_tar, reduction='mean') * self.cfg['loss_ratio'][-1]
        
        # 6.)  CV = H(C|Z, W) = E_q(z,w) [ E_p(c|z,w)[ - log P(c|z,w)] ]
        elbo8   = self.EntropyCriterion()
        ...
        loss_dict['Salient_Unet'] = elbo7
        loss_dict['CV_entropy'] = elbo8
        ...
        if self.include_elbo2:
            loss_dict['loss'] = elbo1 + elbo2 + elbo3  + elbo5 + elbo6 + elbo7 +elbo8
        else:
...
inv_normalize = transforms.Normalize(
    mean=[-4467/0.2603, -0.4398/0.2566, -0.4066/0.2713],
    std=[1/0.2603, 1/0.2566, 1/0.2713]
)
#########################################
...
writer.add_image('STL10_images', inv_normalize(img_grid))
writer.close()
###*********************************************************###
...
net = InfGaussMMVAE(hyperParams, 25, 3, 200, 64, 500, device, raw_img_width, hyperParams["batch_size"],include_elbo2=True)
...
#print(summary(net, (3, img_width, img_width)))
###*********************************************************###
...
salient_en    = torch.optim.SGD(net.encoder_w.parameters(), lr=learning_rate, momentum=0.9, weight_decay=0.0005)
salient_dec   = torch.optim.SGD(net.decoder_w.parameters(), lr=learning_rate * 10, momentum=0.9, weight_decay=0.0005)
...
def train(epoch):
    ...
    return train_loss_avg, X_recons_linear, loss_dict['kumar2beta_kld'].item(), loss_dict["wasserstein_loss"].item(), loss_dict['z_latent_space_kld'].item(), loss_dict['Salient_Unet'].item()
    ...
    with torch.no_grad():
        for i, (image_batch, _) in enumerate(test_loader):
            ...
            if i == 0 and epoch % 20 == 0:
                ...
                image_numpy = inv_normalize(image_batch[-1]).cpu().float().numpy()
                ...
                post_transform = transforms.Compose([
                     ...
                     transforms.Resize((image_numpy.shape[1], image_numpy.shape[0]))])
                #based on this: https://github.com/sunset1995/CVFX-HW5/blob/517be65747bb1e14d9cb68a2d35d1f25d05d4300/saliency.py#L69
                
                img = net.pred[-1][-1].cpu().float().numpy()
                salience_numpy = post_transform(img.squeeze(0))
                
                writer.add_image('Salience images',img, epoch) 
                sal_img = np.array(salience_numpy)
                
                fig = plt.figure(dpi=600)
                plt.imshow(sal_img)
                plt.imsave(str(Path().absolute())+"/results/Salience_images_Hierarchical_StickBreaking_GMMVAE_"+str(epoch)+"_epochs.png", sal_img)
                """
                plotter.add_image(
...
for file in os.listdir(str(Path().absolute())+"/results/"):
    if file.startswith("model_Hierarchical_StickBreaking_GMMVAE_") and file.endswith(".pth"):
        ...
        salient_en.load_state_dict(checkpoint['salient_encoder_state_dict'])
        salient_dec.load_state_dict(checkpoint['salient_decoder_state_dict'])
...
for epoch in range(start_epoch, num_epochs):
        ...
        average_epoch_loss, out , elbo2, wasserstein_loss, latent_dimension_kld, salient_loss = train(epoch)
        if (epoch % 10 == 0):
            test(epoch)
        ...
        if (epoch % 20 == 0) and (epoch//20> 0):
            torch.save({
                    ...
                    'salient_encoder_state_dict':copy.deepcopy(salient_en.state_dict()),
                    'salient_decoder_state_dict':copy.deepcopy(salient_dec.state_dict()),
        ...
        writer.add_scalar('Training salient network loss', float(salient_loss), epoch)
writer.flush()
