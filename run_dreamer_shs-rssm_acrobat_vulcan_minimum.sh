...
#SBATCH --output=/home/memole/scratch/AIME/logs/dreamerv3-shs-rssm-acrobot_%A_%a.out
#SBATCH --error=/home/memole/scratch/AIME/logs/dreamerv3-shs-rssm-acrobot_%A_%a.err
...
SHS_Q_RANK="${SHS_Q_RANK:-2}"
SHS_ACTION_DIM="${SHS_ACTION_DIM:-1}"
FLAGS="${FLAGS:-}"

LOGDIR="${LOGDIR:-${PROJECT_DIR}/logdir/dmc_acrobot_shs_seed_${SEED}}"
...
echo "Acrobot:   seed=${SEED} steps=${STEPS} batch=${BATCH_SIZE}x${BATCH_LENGTH}"
...
suite.load("acrobot", "swingup").physics.render(64, 64)
...
python -u dreamer.py --configs dmc_vision dmc_acrobot_shs \
  --seed "${SEED}" \
  --compile False \
