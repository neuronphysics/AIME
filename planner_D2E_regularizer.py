...
class D2EAgent(Agent):
  ...
  def _build_q_loss(self, batch):
    ...
    q1_target = r.to(device=self.device) + (1.0 - done.float().to(device=self.device)) * self._discount *  torch.minimum(target_v_next, real_v_target) #Q(s,a)=R(s,a)+discount*v(s')
    q_losses = []
