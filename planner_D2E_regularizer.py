...
class D2EAgent(Agent):
    ...
    def _build_q_loss(self, batch):
        ...
        q1_target = r.to(device=self.device) + (
                1.0 - done.float().to(device=self.device)) * self._discount * torch.minimum(target_v_next,
                                                                                            real_v_target)  # Q(s,a)=R(s,a)+discount*v(s')
        q_losses = []
...
def eval_policy_episodes(env, policy, n_episodes):
    ...
    for _ in range(n_episodes):
        ...
        while not time_step.is_last():
            action = policy(torch.from_numpy(time_step.observation))
            if action.ndim < 1:
                time_step = env.step(action.unsqueeze(0).detach().cpu())
            elif action.ndim > 1:
                time_step = env.step(action.detach().cpu()[0])
            else:
