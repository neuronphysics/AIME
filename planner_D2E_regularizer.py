...
class ValueNetwork(nn.Module):
    ...
    def __init__(
      ...
      self.checkpoint_file = os.path.join(self.checkpoint_dir, name + '_D2E')
...
def get_modules(model_params, action_spec):
  """Gets pytorch modules for Q-function, policy, value and discriminator."""
...
class Agent(object):
  ...
  def __init__(
      ...
      time_step_spec=None,
      modules=None,
      optimizers= ((0.001, 0.5, 0.99),),#learning rate, beta parameters
      batch_size=64,
      weight_decays=(0.0,),
      update_freq=1,
      update_rate=0.005,
  ...
  def _build_agent(self):
    ...
    self._global_step = torch.autograd.Variable(torch.tensor(0.0, requires_grad=True))
  ...
  def _tensor(self, x):
      return to_tensor(x, self._device)
...
class AgentModule(nn.Module):
  ...
  def _build_modules(self):
    ...
    self._alpha_var = torch.autograd.Variable(torch.tensor(1.0, requires_grad=True))
    self._alpha_entropy_var = torch.autograd.Variable(torch.tensor(1.0, requires_grad=True))
  ...
  def v_weights(self):
    v_weights=[]
    for _, net in self._v_net._modules.values():
            v_weights += net.weight.data
    return v_weights

  @property
  def v_variables(self):
    vars = self._v_net.parameters()
    vars_=[]
    for v in vars:
        if v.requires_grad:
           vars_.append(v)   
  ...
  def p_weights(self):
    p_weights=[]
    for _, net in self._p_net._modules.values():
            p_weights += net.weight.data
    return p_weights
  ...
  def c_weights(self):
    c_weights=[]
    for _, net in self._c_net._modules.values():
            c_weights += net.weight.data
    return c_weights
...
class D2EAgent(Agent):
  ...
  Divergence between two transition function is computed using the Expected Information Maximization algorithm 
  suggested in this paper https://arxiv.org/pdf/2001.08682.pdf
  ...
  def __init__(
      ...
      dyamics_divergence=0.0, #New term to include KL divergence of transition funnction 
      alpha_entropy=0.0,
    ...
    self._dynamic_divergence = dyamics_divergence #KL term for the transition func.
  ...
  def _build_fns(self):
    ...
    self._v_fn = self._agent_module.v_fn #introducing separate value network for D2E agent
    self._divergence = utils.get_divergence(
        name=self._divergence_name)
    ### need to set this function to estimate KL divergence of the transition function????
    self._EIM =   
  ...
  def _build_q_loss(self, batch):
    ...
    r = batch['r']
    dsc = batch['dsc']
    _, a2_p, _ = self._p_fn(s2) #a_sample, log_probability (a_sample)
    q2_targets = []
    q1_preds = []
  ...
  def _build_p_loss(self, batch):
    ...
    p_loss = torch.mean(
        ...
        + self._get_alpha() * div_estimate + v_target
        - q1 * q_start)
  ...
  def _build_v_loss(self, batch):
    ...
    r   = batch['r']
    dsc = batch['dsc']
    _, a_p, _ = self._p_fn(s1)
    v_target = self._v_fn(s2)
    
    div_estimate = self._divergence.dual_estimate(
        s1, a_p, a_b, self._c_fn)
    ##########complete here #############
    EIM_estimate = self._EIM.(s2,s1) #need to be fixed???
    #Equation 20
    v_loss = torch.mean(r - dsc * self._discount * v_target
        - self._get_alpha() * (div_estimate 
        + EIM_estimate))
    v_w_norm = self._get_v_weight_norm()
    norm_loss = self._weight_decays[1] * v_w_norm
  ...
  def _build_optimizers(self):
    ...
    self._v_optimizer = utils.OptMirrorAdam(self._get_v_vars(),lr=opts[3][0], betas=(opts[3][1],opts[3][2]), weight_decay=self._weight_decays[3])
  ...
  def train_step(self):
    ...
    for _ in range(self._c_iter - 1):
      train_batch = self._get_train_batch()
      info["critic_loss"]=self._extra_c_step(train_batch)
  ...
  def _build_checkpointer(self):
      checkpoint = {
            ...
            "value_net": self._v_fn.state_dict(),
  ...
  def _load_checkpoint(self):
        ...
        self._v_fn.load_state_dict(checkpoint["value_net"])
