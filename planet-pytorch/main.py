...
metrics = {'steps': [], 'episodes': [], 'train_rewards': [], 'test_episodes': [], 'test_rewards': [], 'observation_loss': [], 'reward_loss': [], 'action_loss': [], 'transition_loss': []}
...
for episode in tqdm(range(metrics['episodes'][-1] + 1, args.episodes + 1), total=args.episodes, initial=metrics['episodes'][-1] + 1):
  ...
  for s in tqdm(range(args.collect_interval)):
    ...
    action_loss = F.mse_loss(posterior_actions, actions[args.lagging_size:-1].unfold(0, args.horizon_size, 1).permute(3, 0, 1, 2), reduction='none').sum(dim=(2, 3)).mean(dim=(0, 1))
    transition_loss = F.mse_loss(posterior_states, latent_states[args.lagging_size:-1].unfold(0, args.horizon_size, 1).permute(3, 0, 1, 2), reduction='none').sum(dim=(2, 3)).mean(dim=(0, 1))
    ...
    (observation_loss + reward_loss + action_loss + transition_loss).backward()
  ...
  with torch.no_grad():
    ...
    for t in pbar:
      ...
      if time_step < args.lagging_size:
        ...
        lagging_states = torch.cat([lagging_states[1:], current_latent_state.unsqueeze(dim=0)], dim=0)
