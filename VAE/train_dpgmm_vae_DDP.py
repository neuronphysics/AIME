...
# Configure logging
logging.basicConfig(level=logging.INFO) # format=f"[{rank}/{world_size}] %(name)s - %(message)s ",
...
def compute_metrics(model, test_loader, device, writer, global_step, epoch=None, max_samples=100, visualize_tsne=False, result_dir=None):
...
class MNISTAugmented(torchvision.datasets.MNIST):
    def __init__(
        ...
        self.transform = transforms.Compose([
            ...
            transforms.Grayscale(num_output_channels=3),  # Convert MNIST to 3-channel (RGB) images
            transforms.ToTensor(),  # Convert images to tensors
            transforms.Normalize(mean=[0.1307, 0.1307, 0.1307], std=[0.3081, 0.3081, 0.3081]),  # Normalize as RGB channels
        ])
    ...
    def __getitem__(self, idx):
        ...
        mask = (segmentation_mask > 0).float()
        mask_onehot = torch.zeros((2, mask.shape[1], mask.shape[2]))  # Two channels for background and foreground
        mask_onehot[0] = 1.0 - mask.squeeze(0)  # Background
        mask_onehot[1] = mask.squeeze(0)        # Foreground
...
def main():
    ...
    # Load dataset
    train_dataset = MNISTAugmented(
        root='~/scratch/datasets',
        split="trainval",
        download=False
    )
    
    test_dataset = MNISTAugmented(
        root='~/scratch/datasets',
        split='test',
        download=False
    )
