"""End-to-end SHS-RSSM disentanglement demo on the moving-shapes dataset.

This runs WITHOUT training DreamerV3. It (1) generates shape videos with ground-truth
factors, (2) extracts a per-frame feature with a *random* (untrained) conv stack -- a
stand-in for an untrained DreamerV3 encoder, so any structure found is genuinely
discovered, not hand-crafted -- (3) uses the sticky-HDP switching regime model and the
birth/merge/delete MOVES to discover how many dynamics regimes the feature sequence needs,
and (4) produces the t-SNE / mutual-information / alignment diagnostics.

The point is to validate the pipeline and show the FORMAT of the evidence. With a random
encoder the alignment is already non-trivial; a trained DreamerV3 encoder (use
`task: shapes_four`, `use_shs: True`, `shs_diag_log: True`) makes the latent far cleaner.

Run:  PYTHONPATH=. python3 demo_shapes_disentangle.py
"""
import os
...
def pixel_pca_features(frames, out_dim=24, down=10, seed=0):
    """Per-frame feature from coarse downsampled pixels + PCA. Downsampling to a small
    grid preserves the color and rough position of each shape (what distinguishes the
    scene composition) while discarding fine detail -- a fair, NON-hand-crafted proxy for
    the kind of representation a trained encoder learns. frames: (B,T,H,W,3) uint8 ->
    Z: (B,T,out_dim) float64, standardized."""
    import torch.nn.functional as Fnn
    x = torch.from_numpy(frames.astype(np.float32) / 255.0)        # (B,T,H,W,3)
    B, T, H, W, C = x.shape
    x = x.permute(0, 1, 4, 2, 3).reshape(B * T, C, H, W)
    x = Fnn.adaptive_avg_pool2d(x, (down, down)).reshape(B * T, -1)  # (B*T, 3*down*down)
...
def random_conv_features(frames, out_dim=24, seed=0):
    """Per-frame feature from a fixed random (untrained) 2-layer conv + global pool, then
    PCA. Kept as an 'untrained encoder' baseline; random mixing scrambles color, so it is
    a weaker representation than pixel_pca_features. frames: (B,T,H,W,3) uint8."""
...
def discover_regimes(Z, K_init=2, n_sweeps=6, refine_iters=4, seed=0):
    """Fit the SHS regime model to the feature sequence, using the moves to find K.

    Pure switching-AR on Z (deter = 0), so regimes are discovered from the latent
    dynamics alone. Returns (head, gamma)."""
    torch.manual_seed(seed)
    B, T, L = Z.shape
    H = 4
    deter = torch.zeros(B, T, H, dtype=DT)                         # no exogenous input
    ...
    # seed from a 2-means split so the AR fit is not degenerate, then refine
    ...
    from shs_rssm.moves import _current_stats  # noqa
    ...
    # relax to a fixed point, then let the moves grow / prune K
...
def main():
    ...
    # an evolution strip: latent t-SNE coloured by composition as the representation's
    # CAPACITY grows (pixel-PCA dim 2 -> 24). This is an honest proxy for encoder training
    # -- different features per panel, so the layout genuinely changes -- showing the
    # composition gradient emerge as the representation gets richer. During real training
    # the hook (save_latent_snapshot in models.py) feeds actual checkpoints here instead.
    snaps = []
