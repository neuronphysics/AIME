...
def beta_fn(a,b):
    global local_device
    return torch.exp(torch.lgamma(a.clone().detach().requires_grad_(True).to(device=local_device)) + torch.lgamma(b.clone().detach().requires_grad_(True).to(device=local_device)) - torch.lgamma((a+b).clone().detach().requires_grad_(True).to(device=local_device)))
