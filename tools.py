...
def sample_whole_episodes(episodes, n, max_len=500, seed=0):
    """Sample n COMPLETE episodes for frozen-representation regime consolidation.

    Each episode is returned as its own (1, T, ...) batch dict in the same numpy format the
    training dataset yields (so WorldModel.preprocess/encoder handle it unchanged), truncated
    to its first `max_len` steps so is_first[0] is a real episode boundary. Unlike
    sample_episodes, this never splices fragments of different episodes together: the regime
    HMM transition counts need complete, un-spliced trajectories.
    """
