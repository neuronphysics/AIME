    ...
    while (steps and step < steps) or (episodes and episode < episodes):
        ...
        if done.any():
            ...
            for i in indices:
                ...
                if on_episode_complete is not None and not is_eval:
                    try:
                        _elen = len(cache[envs[i].id]["reward"])
                        _key = f"{envs[i].id}-{_elen}"
                        on_episode_complete(envs[i].id, _key, cache[envs[i].id])
                    except Exception:
                        import traceback
                        import warnings
                        simulate._episode_hook_errors = getattr(
                            simulate, "_episode_hook_errors", 0) + 1
                        if simulate._episode_hook_errors == 1:
                            warnings.warn(
                                "on_episode_complete hook failed; this episode was NOT "
                                "enqueued for SHS streaming (further failures counted in "
                                "tools.simulate._episode_hook_errors). First traceback:\n"
                ...
                if not is_eval:
                    ...
                    if "context_id" in cache[envs[i].id]:  # stable id from the configured context set
                        ctx = int(np.asarray(cache[envs[i].id]["context_id"])[0])
                    ...
                    if len(eval_scores) >= episodes and not eval_done:
                        if eval_ctx_scores:
                            ...
                            for ctx, v in sorted(eval_ctx_scores.items()):
                                logger.scalar(f"eval_return_ctx{ctx}", float(np.mean(v)))
                                logger.scalar(f"eval_episodes_ctx{ctx}", len(v))
...
def episode_to_batch(ep, max_len=500):
    if ep is None:
        return None
    total = len(next(iter(ep.values())))
    if total < 2:
        return None
    T = min(total, max_len)
    d = {k: np.asarray(v[:T])[None] for k, v in ep.items() if "log_" not in k}
    if "is_first" in d:
        d["is_first"][0, 0] = True
    return d


def episode_to_chunks(ep, chunk_len=500):
    if ep is None:
        return None
    total = len(next(iter(ep.values())))
    if total < 2:
        return None
    cl = max(2, int(chunk_len))
    out = []
    for s in range(0, total, cl):
        e = min(total, s + cl)
        d = {k: np.asarray(v[s:e])[None] for k, v in ep.items() if "log_" not in k}
        if "is_first" in d:
            d["is_first"][:] = False
            if s == 0:
                d["is_first"][0, 0] = True
...
def sample_whole_episodes(episodes, n, max_len=500, seed=0):
    rng = np.random.RandomState(seed)
    vals = list(episodes.values())
    order = list(rng.permutation(len(vals)))   # unique-first selection
    out, tries = [], 0
    while len(out) < n and tries < 10 * max(n, 1):
        ...
        if not vals:
            break
        if order:
            ep = vals[int(order.pop())]
        else:   # fewer usable episodes than requested: fall back to repeats
            ep = vals[rng.randint(len(vals))]
        total = len(next(iter(ep.values())))
        if total < 2:
            continue
        T = min(total, max_len)
    ...
    while True:
        ...
        while size < length:
            ...
            if not ret:
                ...
                if "is_first" in ret:
                    ret["episode_first"] = ret["is_first"].copy()
                    ret["is_first"][0] = True
            else:
                index = 0
                possible = length - size

                _epf = ret.get("episode_first", None)
                ret = {
                    k: np.append(
                ...
                if _epf is not None and "is_first" in episode:
                    ret["episode_first"] = np.append(
                        _epf,
                        episode["is_first"][index : min(index + possible, total)].copy(),
                        axis=0,
                    )
                if "is_first" in ret:
...
def load_episode_by_key(directory, key):
    directory = pathlib.Path(directory).expanduser()
    matches = sorted(directory.glob(f"{key}.npz"))
    if not matches:
        return None
    with matches[0].open("rb") as f:
        episode = {k: v for k, v in np.load(f, allow_pickle=True).items()}
    return episode
