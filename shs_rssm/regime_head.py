...
class RegimeHead(nn.Module):
    def __init__(
        ...
        rstick_hier: bool = True,            # hierarchical N(mu, 1/lam) prior on the gate bias
        rstick_hier_a0: float = 2.0,         # Gamma shape on lam (bias precision)
        rstick_hier_b0: float | None = None, # Gamma rate; None -> a0 * rstick_bias_var
        rstick_hier_used_only: bool = False, # hyper-update from rows with gate evidence only
        rstick_hier_min_evidence: float = 1.0,
        gate_input: str = "carry",           # 'carry': phi=[tanh(P h),a,1] (shipped)
                                             # 'latent': phi=[z_{t-1},a,1]  (z -> s edge)
        gate_inner_iters: int = 1,           # >1 is experimental; see regime_inference
        ...
        self._ctor_kwargs = dict(
            ...
            rstick_hier=rstick_hier, rstick_hier_a0=rstick_hier_a0,
            rstick_hier_b0=rstick_hier_b0, rstick_hier_used_only=rstick_hier_used_only,
            rstick_hier_min_evidence=rstick_hier_min_evidence,
        ...
        if self.gate_input not in ("carry", "latent"):
            raise ValueError(f"gate_input must be 'carry' or 'latent', got {gate_input!r}")
        if self.gate_input == "latent":
            # phi = [z_{t-1}, a_{t-1}, 1]: the gate reads the CONTINUOUS LATENT,
            # giving a genuine z -> s edge.  The parent appends the bias slot,
            # so feat_dim is L + action width.
            from .latent_stick import LatentStickiness
            self.rstick = LatentStickiness(
                K=K, feat_dim=self.L + self.rstick_action_dim, latent_dim=self.L,
                prior_persist=prior_persist, pg_iters=pg_iters,
                weight_prior_var=rstick_weight_var, bias_prior_var=rstick_bias_var,
                hier_bias=rstick_hier, hier_a0=rstick_hier_a0, hier_b0=rstick_hier_b0,
                hier_used_only=rstick_hier_used_only,
                hier_min_evidence=rstick_hier_min_evidence,
    ...
    def build_stick_phi_z(self, z_mean, z_var, action=None, is_first=None):
        """Packed gate features for the LATENT gate: phi_t = [z_{t-1}, a_{t-1}, 1].

        Returns cat([E[phi], diag Cov[phi]], -1); LatentStickiness unpacks it.
        The z block is SHIFTED by one step and reset at episode starts, exactly
        as build_g does, so the gate and the dynamics see the same predecessor.
        """
        zm = self._prev_stoch(z_mean, is_first)
        zv = (self._shift_var(z_var, is_first) if z_var is not None
              else torch.zeros_like(zm))
        act = None
        if self.rstick_action_dim > 0:
            act = (self._shift_action(action, is_first) if action is not None
                   else zm.new_zeros(*zm.shape[:-1], self.rstick_action_dim))
        pb, s2 = self.rstick.build_phi_moments(zm, zv, act)
        return self.rstick.pack(pb, s2)

    def _prev_stoch(self, stoch, is_first):
        B, T, L = stoch.shape
        # q(z0) is marginalised ANALYTICALLY: its variance enters through
        # _shift_var/_shift_cov and the smoother, and its KL through z0_kl().
        # The mean is used here; sampling as well would double-count v0.
        z0 = self.z0.view(1, 1, L)
        z0 = z0.to(dtype=stoch.dtype, device=stoch.device).expand(B, 1, L)
        prev = torch.cat([z0, stoch[:, :-1]], dim=1) if T > 1 else z0
        if is_first is not None:
            isf = is_first.reshape(B, T, 1).to(stoch.dtype)
            prev = torch.where(isf > 0.5, z0[:, :1], prev)
        return prev
    ...
    def _gate_phi(self, deter, action=None, is_first=None, z_mean=None, z_var=None,
                  dtype=None):
        """Gate features, from the carry or from the latent depending on gate_input."""
        if getattr(self, "gate_input", "carry") == "latent":
            if z_mean is None:
                raise ValueError(
                    "gate_input='latent' needs z_mean (and ideally z_var): the gate "
                    "reads the continuous latent, not the deterministic carry")
            phi = self.build_stick_phi_z(z_mean, z_var, action, is_first=is_first)
        else:
            phi = self.build_stick_phi(deter, action, is_first=is_first)
        return phi if dtype is None else phi.to(dtype)

    def _transition_logpotentials(self, deter, dtype=None, device=None, action=None,
                                  is_first=None, z_mean=None, z_var=None):
        ...
        if self.recurrent:
            phi = self._gate_phi(deter.to(dtype), action, is_first, z_mean, z_var, dtype)
    ...
    def _transition_potentials_ondemand(self, deter, dtype=None, device=None, action=None,
                                        is_first=None, z_mean=None, z_var=None):
        dtype = dtype if dtype is not None else deter.dtype
        device = device if device is not None else deter.device
        log_init = self.hdp.expected_log_init().to(dtype=dtype, device=device)
    ...
    def _vb_evidence(self, z_mean, deter, is_first=None, z_var=None, action=None,
                     z_cov=None, zg_xcov=None, prev_stoch=None, detach_prev=False):
        """detach_prev stops the gradient through the PREDECESSOR inside g without
        changing anything else.  It exists because reusing `prev_stoch` for that
        purpose also flips _cond_prev, which drops g_var/g_zcov and changes the
        evidence by O(1) -- a silent semantic change, not a detach."""
        ...
        if detach_prev:
            # sg(prior) must stop the gradient through EVERYTHING the predecessor
            # contributes: its mean (above) and its variance/covariance, which
            # enter the prior through g_var / g_zcov.
            gvar = None if gvar is None else gvar.detach()
            g_zcov = None if g_zcov is None else g_zcov.detach()
    ...
    def regime_inference(self, stoch, deter, is_first=None, cache_estep: bool = False,
                         ...
                         prev_stoch=None, inner_iters=None, episode_first=None):
        """E-step over the discrete chain.

        With gate_input='carry' the transition potentials are CONSTANT in the
        continuous latent, so one pass is exact and `inner_iters` is ignored.

        Discrete E-step.  `gate_inner_iters` > 1 (gate_input='latent' only) is
        EXPERIMENTAL and OFF BY DEFAULT; read this before turning it on.

        At fixed q(beta) one pass is already exact: the JJ tilt is
        c = sqrt(E[psi]^2 + Var[psi]), which depends on q(beta) and phi but not
        on the responsibilities, so the potentials do not move and a second
        forward-backward returns the same q(s).  Extra passes therefore only do
        something if q(beta) itself changes, and what this loop does is refit
        q(beta) on THIS batch alone (prior + batch statistics), rebuild the
        potentials, re-run the chain, then restore the persistent gate.

        That is a batch-adaptive procedure, NOT coordinate ascent on the global
        bound: the returned responsibilities are optimal for the temporary gate,
        not for the persistent gate that subsequent scoring and prediction use,
        and the chain normaliser reported under the temporary gate is not the
        persistent model's normaliser.  Measured on a fixed batch, the two-pass
        factor scores WORSE under the restored gate than the one-pass factor.
        It also breaks the equivalence that lets the S-1 extra posterior draws
        be batched, because one temporary gate is then shared across draws.

        Use it only as a deliberate experiment with its own objective; leave it
        at 1 for anything whose bound or statistics are being compared.
        """
        _latent = getattr(self, "gate_input", "carry") == "latent" and self.recurrent
        n_it = int(inner_iters if inner_iters is not None
                   else getattr(self, "gate_inner_iters", 1))
        if not _latent or n_it <= 1 or getattr(self, "rstick", None) is None:
            self._last_inner_iters = 1
            return self._regime_inference_once(
                stoch, deter, is_first=is_first, cache_estep=cache_estep, z_var=z_var,
                action=action, valid=valid, z_cov=z_cov, zg_xcov=zg_xcov,
                prev_stoch=prev_stoch, episode_first=episode_first)

        rs = self.rstick
        _saved = {k: getattr(rs, k).detach().clone()
                  for k in ("pg_A", "pg_h", "m_beta", "Sigma_beta", "m0", "sigma0_diag")
                  + tuple(getattr(rs, "_HYPER_BUFFERS", ()))}
        _saved_init = bool(rs._pg_init)
        _prev_estep = getattr(self, "_estep", None)
        try:
            out = None
            for _i in range(n_it):
                out = self._regime_inference_once(
                    stoch, deter, is_first=is_first, cache_estep=True, z_var=z_var,
                    action=action, valid=valid, z_cov=z_cov, zg_xcov=zg_xcov,
                    prev_stoch=prev_stoch, episode_first=episode_first)
                self._last_inner_iters = _i + 1
                if _i == n_it - 1:
                    break
                es = getattr(self, "_estep", None)
                if es is None:
                    break
                with torch.no_grad():
                    K_ = es["r_mass"].shape[-1]
                    st = rs.pg_stats_from_batch(
                        es["phi_steps"].reshape(-1, es["phi_steps"].shape[-1]),
                        es["r_mass"].reshape(-1, K_), es["row_weight"].reshape(-1, K_))
                    rs.pg_set_totals(st["A"], st["h"])     # batch-local posterior
            _final_estep = getattr(self, "_estep", None)
        finally:
            with torch.no_grad():
                for k, v in _saved.items():
                    getattr(rs, k).copy_(v)
                rs._pg_init = _saved_init
        self._estep = _final_estep if cache_estep else _prev_estep
        return out

    def _regime_inference_once(self, stoch, deter, is_first=None, cache_estep: bool = False,
                         z_var=None, action=None, valid=None, z_cov=None, zg_xcov=None,
                         prev_stoch=None, episode_first=None):
        with torch.amp.autocast("cuda", enabled=False):
            ...
            zcov = _c(z_cov)
            zxc = _c(zg_xcov)
            # Drop any cache from a previous call: it is keyed only by shape, so
            # a stale entry from an earlier batch would otherwise be reused as
            # this batch's pairwise marginals in the discrete-path cost.
            self._struct_cache = None
            _evm = self._crop_ev_mask(is_first, z.shape[0], z.shape[1],
                                      z.dtype, z.device, episode_first=episode_first)
            _hmm_isf, _sa0 = self._hmm_boundary(is_first, episode_first)
            ...
            if _online_pw:
                ...
                gamma, counts_base, _logZ_re, _la, _lb = forward_backward(
                    log_init.double(), None, ev.double(), is_first=_hmm_isf, valid=valid,
                    ev_mask=_evm, assume_start_at_t0=_sa0,
                ...
                gamma, counts_base, _logZ_re, xi = forward_backward(
                    ...
                    is_first=_hmm_isf, valid=valid, ev_mask=_evm, return_pairwise=True,
                    assume_start_at_t0=_sa0)
                gamma = gamma.to(ev.dtype)
                counts_base = counts_base.to(ev.dtype)
                _logZ_re, xi = _logZ_re.to(ev.dtype), xi.to(ev.dtype)
            _vsrc = _evm if valid is None else (
                valid if _evm is None else
                _evm.reshape(valid.shape).to(valid.dtype) * valid)
            if _vsrc is not None:
                _vmask = _vsrc.reshape(_ev_shift.shape[0], _ev_shift.shape[1]).to(_ev_shift.dtype)
            else:
            ...
            if self.recurrent:
                if _online_pw:
                    ...
                    r_mass, row_weight, counts = accumulate_pg_online(
                        _la, _lb, _logZ_re, _trans_fn,
                        (ev if _evm is None else
                         torch.where(_evm.reshape(*ev.shape[:2], 1) > 0.5,
                                     ev, torch.zeros_like(ev))),
                        _hmm_isf, valid, trans_aux)
                else:
        ...
        start_counts = start_counts_from(gamma, _hmm_isf, valid=valid,
                                         assume_start_at_t0=_sa0)
        return gamma, counts, start_counts, g

    def _discrete_path_kl(self, gamma, xi, deter, is_first=None, action=None,
                          z_mean=None, z_var=None, episode_first=None, valid=None):
        B, T, K = gamma.shape
        dtype, device = gamma.dtype, gamma.device
        log_init = self.hdp.expected_log_init().to(dtype=dtype, device=device)
        _f = episode_first if episode_first is not None else is_first
        if _f is None:
            isf = torch.zeros(B, T, dtype=dtype, device=device)
        else:
            isf = _f.reshape(B, T).to(dtype=dtype, device=device).clone()
        if episode_first is None:        
           isf[:, 0] = 1.0
        ...
        out = out + isf * init_kl
        # A replay-crop boundary is not a genuine episode start, but its initial
        # discrete factor still has to be scored: q(s_0) is refined against the
        # uninformative boundary prior, so the fragment cost includes
        # KL(q(s_0) || Uniform).  Omitting it under-reports the structured bound.
        # Every RETAINED FRAGMENT has an initial factor, not just t=0: after an
        # interior gap the chain restarts, and that restart is scored against the
        # learned pi0 at a genuine episode start or against Uniform otherwise.
        _lu = -float(math.log(K))
        _uni = (gamma * (gclamp.log() - _lu)).sum(-1)
        _frag = torch.zeros(B, T, dtype=dtype, device=device)
        _frag[:, 0] = 1.0
        if valid is not None:
            _v = valid.reshape(B, T).to(dtype)
            _frag[:, 0] = _v[:, 0]
            if T > 1:
                _frag[:, 1:] = torch.maximum(_frag[:, 1:],
                                             (_v[:, 1:] * (1.0 - _v[:, :-1])).clamp(0.0, 1.0))
        # genuine starts already paid init_kl above; the rest pay the uniform one
        out = out + _frag * (1.0 - isf) * _uni
        ...
        base_elogpi = self.hdp.expected_log_trans().to(dtype=dtype, device=device)
        if self.recurrent:
            phi = self._gate_phi(deter, action, is_first, z_mean, z_var)
    ...
    def _discrete_path_kl_online(self, gamma, deter, is_first, cache, action=None,
        ...
        log_init = self.hdp.expected_log_init().to(dtype=dtype, device=device)
        _f = episode_first if episode_first is not None else is_first
        isf = (torch.zeros(B, T, dtype=dtype, device=device) if _f is None
               else _f.reshape(B, T).to(dtype=dtype, device=device).clone())
        if episode_first is None:
            isf[:, 0] = 1.0      
        gclamp = gamma.clamp_min(1e-30)
        out = gamma.new_zeros(B, T)
    ...
    def dynamics_kl(self, q_mean, q_std, stoch, deter, gamma, is_first=None,
                    ...
                    prev_stoch=None, valid= None, episode_first=None):
        q_var = (q_std ** 2).clamp_min(1e-8)
        gam = gamma.detach()
        _valid = self._chunk_valid(valid, is_first, q_mean.shape[0],
                                   q_mean.shape[1], q_mean.dtype, q_mean.device,
                                  episode_first=episode_first)
        # DreamerV3 eq. 5 routes gradients through TWO KLs with opposite
        # stop-gradients 
        #   rep = KL[q || sg(p)]  -> encoder only
        #   dyn = KL[sg(q) || p]  -> prior/dynamics only
        # In SHS the regime parameters (A_k, Q_k) are fitted by the conjugate
        # M-step and take no gradient, so the only "prior" pathway that can be
        # trained is the GRU carry reaching the evidence through
        # g = [z_{t-1}, h_t, a, 1].  `evidence` is kept ungated for the
        # diagnostics and the discrete path below.
        ...
        # sg(p): the prior conditions on BOTH h_t and z_{t-1} through
        # g = [z_{t-1}, h_t, a, 1].  DreamerV3 stops z_{t-1} implicitly by
        # detaching h_t (its only route into the prior); here z_{t-1} enters
        # the prior directly, so it needs its own detach.
        ev_rep, _, _ = self._vb_evidence(q_mean, deter.detach(), is_first=is_first,
                                         z_var=q_var,
                                         prev_stoch=(None if prev_stoch is None
                                                     else prev_stoch.detach()),
                                         detach_prev=True, action=action)
        # sg(q): detach the posterior everywhere it appears, including as the
        # predecessor (relevant when shs_moment_consistent=True passes an
        # attached prev_stoch).
        ev_dyn, _, _ = self._vb_evidence(q_mean.detach(), deter, is_first=is_first,
                                         z_var=q_var.detach(),
                                         prev_stoch=(None if prev_stoch is None
                                                     else prev_stoch.detach()),
                                         detach_prev=True, action=action)
        rep_vb = -(gam * ev_rep).sum(-1) - hq
        dyn_vb = -(gam * ev_dyn).sum(-1) - hq.detach()
        if _valid is not None:
            _vm = _valid.reshape(cont_vb.shape).to(cont_vb.dtype)
            cont_vb = cont_vb * _vm
            rep_vb = rep_vb * _vm
            dyn_vb = dyn_vb * _vm
        ...
        if _c is not None and _c.get("online", False) \
            ...
            disc = self._discrete_path_kl_online(gam, deter, is_first, _c, action=action,
                                                 ...
                                                 episode_first=episode_first, valid=valid)
        else:
            xi = None
            ...
            if xi is None:
                with torch.no_grad():
                    ...
                    log_init, log_trans, _ = self._transition_logpotentials(
                        deter.detach(), dtype=ev.dtype, device=ev.device, action=action,
                        is_first=is_first, z_mean=q_mean.detach(), z_var=q_var.detach())
                    _hisf, _sa0 = self._hmm_boundary(is_first, episode_first)
                    _evm = self._crop_ev_mask(is_first, ev.shape[0], ev.shape[1],
                                              ev.dtype, ev.device, episode_first=episode_first)
                    _, _, _, xi = forward_backward(
                        log_init, log_trans, ev, is_first=_hisf, valid=valid, ev_mask=_evm,
        ...
        # free bits applied SEPARATELY to each branch, elementwise before any
        # reduction -- matching torch.clip(x, min=free) in dreamerv3 architecture.
        _fb = (lambda x: torch.clamp(x, min=free)) if (free is not None and free > 0) \
            else (lambda x: x)
        rep = _fb(rep_vb)
        dyn = _fb(dyn_vb) + disc + rstick_kl + z0kl_b
        vb = cont_vb + disc + rstick_kl + z0kl_b
        value = vb
        loss = dyn_scale * (_fb(dyn_vb) + disc) + rep_scale * rep + rstick_kl + z0kl_b
        if strict_elbo:
    ...
    def svae_local_bound(self, res, deter, is_first=None, action=None, valid=None):
        ...
        log_init, log_trans, _aux = self._transition_logpotentials(
            ...
            z_mean=res["mean"], z_var=res["z_var"])
    ...
    def _crop_ev_mask(self, is_first, B, T, dtype, device, episode_first=None):
        """1 where a timestep's unary dynamics evidence is real, 0 at a replay
        crop boundary (fabricated z0/pi0 reset).  The state still exists there
        and its outgoing transition is legal: see forward_backward's two masks."""
        if not getattr(self, "chunk_boundary_mask", True) or is_first is None:
            return None
        ef = episode_first if episode_first is not None else is_first
        ef0 = ef.reshape(B, T)[:, 0].to(dtype)
        if bool((ef0 > 0.5).all()):
            return None
    ...
    def _hmm_boundary(self, is_first, episode_first):
        """One boundary convention for the discrete chain, shared by training
        inference, move refinement and move acceptance: genuine episode starts
        reset to the learned pi0, replay-crop starts take the uninformative
        boundary (assume_start_at_t0=False)."""
        if episode_first is not None:
            return episode_first, False
        return is_first, not getattr(self, "chunk_boundary_mask", True)

    def _chunk_valid(self, valid, is_first, B, T, dtype, device,
                     episode_first=None):
        # Crop starts are forced is_first=True upstream, so is_first alone
        # cannot distinguish a genuine episode start from a replay-crop
        # boundary.  Gate on episode_first when available: timestep 0 of a
        # mid-episode crop carries a fictional z0/pi0 reset and must not
        # contribute unary evidence.
        if not getattr(self, "chunk_boundary_mask", True) or is_first is None:
            return valid
        ef = episode_first if episode_first is not None else is_first
        ef0 = ef.reshape(B, T)[:, 0].to(dtype)
    ...
    def absorb_episode(self, episode_id, stoch, deter, is_first=None, z_var=None,
        ...
        self.update_globals(stoch, deter, gamma, tc, sc, is_first=is_first,
                            z_var=z_var, batch_id=episode_id, action=action, valid=valid)
    ...
    def __getstate__(self):
        """RegimeHead holds a threading.RLock, which is not picklable, so plain
        copy.deepcopy(head) raises.  torch.save only ever touches state_dict()
        so checkpointing was never affected, but EMATeacher (teacher.py:83) and
        anything using DataParallel deep-copy the module itself.  Drop the lock
        on the way out and rebuild it on the way in."""
        st = self.__dict__.copy()
        st["_commit_lock"] = None
        return st

    def __setstate__(self, state):
        import threading
        self.__dict__.update(state)
        if self.__dict__.get("_commit_lock", None) is None:
            self.__dict__["_commit_lock"] = threading.RLock()
    ...
    def update_globals(self, stoch, deter, gamma, trans_counts, start_counts,
        ...
        valid = self._chunk_valid(valid, is_first, stoch.shape[0], stoch.shape[1],
                                  stoch.dtype, stoch.device, episode_first=episode_first)
        if valid is not None:
        ...
        if self.recurrent and self._estep is not None:
            ...
            B, S, K = r_mass.shape
            # pg_every applies on the EMA path as well; _gstep_count is advanced
            # by _hdp_update_scheduled just above, so the same counter is used.
            _every = int(getattr(self, "pg_every", 1))
            if _every <= 1 or ((int(getattr(self, "_gstep_count", 1)) - 1) % _every) == 0:
    ...
    def _score_potentials(self, deter, hdp, rstick, dtype=torch.float64, action=None,
                          is_first=None, z_mean=None, z_var=None):
        device = deter.device
        ...
        if rstick is not None:
            phi = self._gate_phi(deter, action, is_first, z_mean, z_var).to(dtype)
    ...
    def bound_local(self, stoch, deter, is_first=None, regimes=None, hdp=None,
        ...
        _evm = self._crop_ev_mask(is_first, stoch.shape[0], stoch.shape[1],
                                  stoch.dtype, stoch.device, episode_first=episode_first)
        _hmm_isf, _sa0 = self._hmm_boundary(is_first, episode_first)
        gamma, xicount, logZ, xi = forward_backward(
            log_init, log_trans, ev, is_first=_hmm_isf, valid=valid, ev_mask=_evm,
            assume_start_at_t0=_sa0, return_pairwise=True)
        if aux is not None:
    ...
    def imagine_prior(self, prev_stoch, deter, resp_prev, sample=True, prev_var=None,
                      action=None, mode=None, weights=None):
        """`weights` overrides the predicted regime law instead of advancing
        resp_prev through the transition: used at an episode start, where the
        regime is drawn from the learned initial law and no transition has
        happened yet."""
        _ZMAX, _SMAX = 20.0, 5.0
        ...
        if self.recurrent:
            if getattr(self, "gate_input", "carry") == "latent":
                # z -> s: the transition matrix is a function of the CURRENT
                # latent, so switching stays locked to position in state space
                # instead of drifting on a learned schedule.
                _pv = prev_var if prev_var is not None else torch.zeros_like(prev_stoch)
                _pb, _s2 = self.rstick.build_phi_moments(
                    prev_stoch, _pv,
                    action if self.rstick_action_dim > 0 else None)
                phi = self.rstick.pack(_pb, _s2)
            else:
        ...
        if weights is not None:
            w = self._mask_weights(weights.to(w.dtype).clamp_min(1e-8))
            w = w / w.sum(-1, keepdim=True)
        comp_mean, comp_var = self.regimes.predictive_moments(g, g_var=g_var)
        # Bound the ROLLOUT DISTRIBUTION, not the realised sample: the component
        # means and the mixture scale are limited here, before anything is drawn,
        # so the returned (mean, std) is exactly the Gaussian the sample came
        # from and get_dist(state) is consistent with `stoch`.
        comp_mean = comp_mean.clamp(-_ZMAX, _ZMAX)
        comp_var = comp_var.clamp(max=_SMAX ** 2)
        ...
        # No cap on the MIXTURE scale: the component means and variances are
        # already bounded above, and capping the mixture spread would report a
        # narrower distribution than the one actually sampled when components
        # are far apart.
        std = var.sqrt()
        ...
        if (not sample) or _mode == "actor_moment":
            ...
            if self.regimes.q_rank > 0:
                ...
                # ONE covariance for both the draw and the reported moments: the
                # marginal variance is diag + rowwise |U|^2, so a cap has to
                # rescale BOTH parts, otherwise the low-rank draw adds variance
                # the reported std does not describe.
                marg = sel_d + sel_U.pow(2).sum(-1)
                scale = (_SMAX ** 2 / marg.clamp_min(1e-12)).clamp(max=1.0)
                sel_d = sel_d * scale
                sel_U = sel_U * scale.unsqueeze(-1).sqrt()
                ...
                # report the mixture built from the SAME limited components
                comp_marg = (comp_d + comp_U.pow(2).sum(-1))
                comp_scale = (_SMAX ** 2 / comp_marg.clamp_min(1e-12)).clamp(max=1.0)
                comp_var_s = comp_marg * comp_scale
                var = (w.unsqueeze(-1) * (comp_var_s + dev.pow(2))).sum(-2).clamp_min(1e-8)
                mean = (w.unsqueeze(-1) * comp_mean).sum(-2)
                std = var.sqrt()
            else:
                sel_var = torch.gather(comp_var, -2, sel).squeeze(-2).clamp(max=_SMAX ** 2)
