...
class RegimeHead(nn.Module):
    def __init__(
        ...
        gate_inner_iters: int = 2,           # E-step coupling passes when gate_input='latent'
    ...
    def regime_inference(self, stoch, deter, is_first=None, cache_estep: bool = False,
        ...
        With gate_input='latent' the potentials depend on q(x) and the
        continuous solve depends on q(z), so the two E-steps no longer separate.
        the coupled inner loop (Zoltowski-style vLEM) is NOT yet threaded:
        nothing is passed between passes, so repeated iterations are
        idempotent.  A single E-step is executed and extra requested
        iterations are ignored with a one-time notice.
        """
        ...
        if _latent and n_it > 1:
            if not getattr(self, "_warned_latent_iters", False):
                print("[SHS] gate_input='latent': inner-loop coupling is not "
                      "yet threaded between passes; running a single E-step "
                      f"(requested inner_iters={n_it} ignored).")
                self._warned_latent_iters = True
            n_it = 1
        if not _latent or n_it <= 1:
        ...
        for _i in range(n_it):
            out = self._regime_inference_once(
                stoch, deter, is_first=is_first,
                cache_estep=(cache_estep and _i == n_it - 1), z_var=z_var,
                action=action, valid=valid, z_cov=z_cov, zg_xcov=zg_xcov,
                prev_stoch=prev_stoch, episode_first=episode_first)
    ...
    def _regime_inference_once(self, stoch, deter, is_first=None, cache_estep: bool = False,
        ...
        _epf = episode_first if episode_first is not None else is_first
        start_counts = start_counts_from(
            gamma, _epf, valid=valid,
            assume_start_at_t0=(False if episode_first is not None
                                else not getattr(self, 'chunk_boundary_mask', True)))
        return gamma, counts, start_counts, g

    def _discrete_path_kl(self, gamma, xi, deter, is_first=None, action=None,
                          z_mean=None, z_var=None, episode_first=None):
        B, T, K = gamma.shape
    ...
    def dynamics_kl(self, q_mean, q_std, stoch, deter, gamma, is_first=None,
        ...
        if _c is not None and _c.get("online", False) \
            ...
            disc = self._discrete_path_kl_online(gam, deter, is_first, _c, action=action,
                                                 z_mean=q_mean, z_var=q_var, episode_first=episode_first)
        else:
            xi = None
            ...
            disc = self._discrete_path_kl(gam, xi.to(gam.device, gam.dtype), deter, is_first,
                                          action=action, z_mean=q_mean, z_var=q_var, episode_first=episode_first)
