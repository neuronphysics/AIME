...
class SharedCarryRegimes(nn.Module):
    ...
    @torch.no_grad()
    def _r_index(self, G):
        """Indices of the regression coordinates r = [z_prev, action, 1] inside a
        full g = [z_prev, htil, action, 1] of width G (mirrors _split_g)."""
        L, Lr = self.L, self.Lr
        n_tail = Lr - L                       # action dims + bias
        return torch.cat([torch.arange(0, L), torch.arange(G - n_tail, G)])

    def log_marginal_from_stats(self, N, Sgg, Szg, Szz):
        """Normal-Gamma marginal likelihood of a block under ONE fresh regime,
        with the coefficient prior N(M0, lam0^-1) (M0 = identity on the z block
        when identity_init) -- the same prior the m-step uses.  Accepts stats in
        r coordinates (width Lr) or in full-g coordinates, from which the r
        coordinates are selected (not a prefix)."""
        L, Lr = self.L, self.Lr
        if Sgg.shape[-1] != Lr:
            ridx = self._r_index(Sgg.shape[-1]).to(Sgg.device)
            Sgg_r = Sgg[..., ridx, :][..., :, ridx]
            Szg_r = Szg[..., :, ridx]
        else:
            Sgg_r, Szg_r = Sgg, Szg
        ...
        M0 = self.M0.to(lam0.dtype)
        rhs = (M0 @ lam0).unsqueeze(0) + Szg_r          # prior mean enters here ...
        ...
        m0lam0m0 = torch.einsum("ir,rs,is->i", M0, lam0, M0).reshape(1, L)   # ... and here
        ...
        bN = torch.clamp(b0 + 0.5 * (Szz + m0lam0m0 - MlamM), min=1e-12)
