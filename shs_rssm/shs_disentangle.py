"""Latent-space disentanglement diagnostics for the SHS-RSSM.

The hypothesis these plots test: the sticky-HDP switching dynamics carve the latent space
into regimes that track the SCENE COMPOSITION (which / how many objects are present), and
regime switches align with object appearance / disappearance events. Concretely we
provide:

  * t-SNE of the latents, colored by inferred regime and by a ground-truth factor, with an
    optional temporal-trajectory overlay (how the dynamics move through latent space);
  * a t-SNE EVOLUTION strip across training checkpoints (latent organizing over training);
  * quantitative alignment metrics -- regime <-> composition normalized MI, regime-switch
    vs composition-change boundary F1, and a latent-dim <-> factor mutual-information
    matrix with a MIG-style disentanglement score.

Everything takes plain tensors / numpy plus a factors dict, so it runs offline (the demo)
and inside the DreamerV3 eval loop (the hook in models.py) identically.
"""
...
def tsne_embedding(Z, max_points=2500, perplexity=30.0, seed=0):
    """2D t-SNE of (N, D) latents. Returns (emb (M,2), idx (M,)) where idx are the
    subsampled rows (t-SNE is O(N^2); we cap N for tractability)."""
...
def _discretize(x, bins=8):
    """Map a 1-D continuous array to integer bins (quantile bins, robust to scale)."""
    x = _np(x).reshape(-1)
...
def regime_factor_alignment(gamma, factors, boundary_tol=2):
    """Quantify how well the inferred regimes encode the ground-truth scene structure.

    Args:
        gamma: (B,T,K) regime responsibilities.
        factors: dict with at least 'n_present' (B,T) and optionally per-shape 'present'
            (B,T,n) / 'alpha' / 'x' / 'y'.
        boundary_tol: a regime switch counts as aligned to a composition change if it
            falls within +/- this many steps.
    Returns:
        dict of scalar metrics.
    """
    gamma = _np(gamma)
    B, T, K = gamma.shape
    regime = gamma.argmax(-1)                                  # (B,T)
    npres = _np(factors["n_present"]).astype(int)              # (B,T)

    # regime <-> composition mutual information (normalized)
    nmi = normalized_mutual_info_score(regime.reshape(-1), npres.reshape(-1))

    # regime purity: fraction of timesteps whose regime's majority composition matches
    ...
    # boundary alignment: regime switches vs composition-change events (per sequence)
    tp = fp = fn = 0
    for b in range(B):
        r_sw = np.where(np.diff(regime[b]) != 0)[0] + 1        # regime-switch steps
        n_ch = np.where(np.diff(npres[b]) != 0)[0] + 1         # composition-change steps
        matched_events = set()
        for s in r_sw:
...
def factor_decodability(stoch, factors, key="n_present", k=15, n_splits=3, seed=0):
    """How recoverable a ground-truth factor is from the latent: balanced k-NN accuracy
    of predicting (discretized) `key` from the latents, against the majority-class
    baseline. High accuracy >> baseline means the factor is topologically present in the
    latent (the disentanglement signal the t-SNE shows), independent of whether any single
    dimension isolates it. Returns dict(accuracy, baseline, key)."""
    ...
    for i in range(n_splits):
        ...
        # balanced accuracy over present classes
        cls = np.unique(y[te]); acc = np.mean([(pred[y[te] == c] == c).mean() for c in cls])
        accs.append(acc)
    vals, cnts = np.unique(y, return_counts=True)
    baseline = 1.0 / len(vals)                                  # balanced chance
...
def latent_factor_mi_matrix(stoch, factors, keys=None, bins=8):
    """Mutual information between each latent dimension and each ground-truth factor.

    Returns (M (L, F) normalized MI matrix, factor_names (F,), mig (float)).
    MIG (mutual-information gap): for each factor, (top-MI dim - 2nd-MI dim) / H(factor),
    averaged. Higher => each factor is captured by a single latent dim (disentangled).
    """
    Z = _np(stoch).reshape(-1, _np(stoch).shape[-1])          # (N, L)
    L = Z.shape[1]
    ...
    for key in keys:
        v = _np(factors[key])
        if v.ndim == 2:                                       # scalar factor (B,T)
            cols.append(v.reshape(-1)); names.append(key)
        else:                                                 # per-shape (B,T,n)
            for i in range(v.shape[-1]):
                cols.append(v[..., i].reshape(-1)); names.append(f"{key}{i}")
    F = len(cols)
    Zb = np.stack([_discretize(Z[:, d], bins) for d in range(L)], 1)   # (N,L)
    M = np.zeros((L, F))
    Hf = np.zeros(F)
    for j, col in enumerate(cols):
        ...
        for d in range(L):
            M[d, j] = mutual_info_score(Zb[:, d], cb) / Hf[j]   # normalized by H(factor)
    # MIG
    migs = []
    for j in range(F):
        s = np.sort(M[:, j])[::-1]
...
def plot_tsne_disentangle(stoch, gamma, factors, path, factor_key="n_present",
    ...
    """t-SNE of the latents colored by (a) inferred regime and (b) a ground-truth factor,
    with optional temporal trajectories (consecutive-step segments) on the regime panel.

    stoch : (B,T,L) latents ; gamma : (B,T,K) ; factors : dict (factor_key -> (B,T) or
    (B,T,n) -> summed). Returns the alignment-metrics dict.
    """
    ...
    fac = fac if fac.ndim == 2 else fac.sum(-1)               # scalarize per-shape factor
    facf = fac.reshape(-1)

    emb, idx = tsne_embedding(Zf, max_points=max_points, seed=seed)
    seq_of = np.repeat(np.arange(B), T)                       # which sequence each row is
    t_of = np.tile(np.arange(T), B)                           # which timestep

    fig, axes = plt.subplots(1, 2, figsize=(13.5, 5.6))
    used = np.unique(regime[idx])

    # (a) colored by regime, with trajectory segments within each sequence
    ax = axes[0]
    if trajectories:
        ...
        for b in range(B):
            ...
            for a, c in zip(rows[:-1], rows[1:]):
                if t_of[idx[c]] - t_of[idx[a]] == 1:          # only true consecutive steps
    ...
    # (b) colored by the ground-truth factor
    ax = axes[1]
...
def plot_mi_matrix(stoch, factors, path, keys=None, bins=8,
    ...
    """Heatmap of normalized MI between latent dims and ground-truth factors, plus the
    MIG disentanglement score. Returns (M, names, mig)."""
...
def plot_tsne_evolution(snapshots, path, color_by="regime", factor_key="n_present",
    ...
    """A strip of t-SNE plots from successive stages, showing the latent space organizing
    as the world model learns. Color each panel either by inferred regime (`color_by=
    "regime"`) or by a ground-truth factor (`color_by="factor"`, using `factor_key`).

    snapshots : list of dicts, each {'step'|'label': ..., 'stoch': (B,T,L)} plus 'gamma'
                (B,T,K) if coloring by regime, or 'factors' if coloring by a factor;
                typically saved periodically during training.
    """
    n = len(snapshots)
