"""Diagnostics for the SHS-RSSM world model: latent regime clustering and frame
reconstruction / open-loop imagination.

These work with any trained world model that exposes a `SHSRSSM` dynamics module and a
decoder mapping features -> image. They are deliberately framework-light: pass in the
tensors and a decode callable.
"""
...
def regime_responsibilities(rssm, post, is_first=None):
    """Posterior regime responsibilities gamma (B,T,K) for an observed segment."""
...
def plot_latent_clustering(stoch, gamma, path, title="Latent regime clustering",
                           true_labels=None):
    """2D projection of the latents coloured by inferred regime, plus occupancy and a
    sample regime timeline.

    stoch : (B,T,L) latent samples
    ...
    # 2D embedding via PCA (top-2 principal directions of the latents)
    Zc = Z - Z.mean(0)
    try:
...
def _to_img(x):
    """(H,W,C) or (C,H,W) or (H,W) -> HxW or HxWxC numpy in [0,1], detecting layout.

    Dreamer image tensors are HWC; a naive CHW transpose would mangle them, so the channel
    axis is detected (a length in {1,3,4}) rather than assumed.
    """
    x = x.detach().cpu().float().numpy() if hasattr(x, "detach") else np.asarray(x)
    if x.ndim == 3:
        if x.shape[-1] in (1, 3, 4):                    # already HWC
            return np.clip(x[..., 0] if x.shape[-1] == 1 else x, 0, 1)
        if x.shape[0] in (1, 3, 4):                     # CHW -> HWC
...
def plot_reconstructions(true_frames, recon_frames, path, imagined_frames=None,
    ...
    """Grid: true / reconstructed (/ open-loop imagined) frames over time.

    Each *_frames is (T,C,H,W) or (T,H,W) for a single episode.
    """
    ...
    for r, (name, frames) in enumerate(zip(row_names, series)):
        for c, t in enumerate(idx):
            ax = axes[r, c]
            ti = min(int(t), frames.shape[0] - 1)          # length-safe: never index past
    ...
    if context is not None:
        # mark where open-loop imagination begins, on the last (model-prediction) row
        for c, t in enumerate(idx):
            if t >= context:
