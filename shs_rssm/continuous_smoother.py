...
def build_blocks(pot, enc_mean, enc_prec, prior_mean=None, prior_prec=None,
                 is_first=None, valid=None, trans_prec=None):
    """trans_prec: (B, T, L, L) PSD contribution of the transition potential.

    Zero for the carry gate -- phi(h_t) is constant in z, so the transition
    potential drops out of the continuous-state solve entirely.  Non-zero for
    the latent gate, where psi = beta^T [z_{t-1}, a, 1] is linear in z; under
    Polya-Gamma augmentation the potential is Gaussian and contributes
    sum_k q(z_t=k) E[omega_tk] E[b_k b_k^T].  Because psi depends on a SINGLE
    time index this lands only on the primary block diagonal, so the
    block-tridiagonal structure -- and the O(T) solve -- are preserved.
    """
    ...
    if trans_prec is not None:
        tp = trans_prec.to(dt)
        if valid is not None:
            tp = tp * valid.to(dt).reshape(B, T, 1, 1)
        D = D + 0.5 * (tp + tp.transpose(-1, -2))      # symmetrise round-off
