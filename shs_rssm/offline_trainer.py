...
def fit_offline_corpus(head, corpus=None, laps: int = 10, encode_fn=None,
    ...
    for lap in range(int(laps)):
        ...
        for entry in batches:
            ...
            vmask = entry[5] if len(entry) > 5 else None
            zv = entry[4] if len(entry) > 4 else None
            gamma, counts, sc, _ = head.regime_inference(
                z, h, is_first=isf, z_var=zv, cache_estep=True, valid=vmask)
            head.update_globals(z, h, gamma, counts, sc, is_first=isf,
                                z_var=zv, batch_id=bid, stats_only=True, valid=vmask)
            buf.add(z, h, isf, zv, batch_id=bid, valid=vmask,
        ...
        if sweep_every and (lap + 1) % int(sweep_every) == 0:
            ...
            if resync_after_moves and any(bool(a) for (a, _g) in log.values()):
                head.stat_store.reset()
                for entry in batches:
                    bid, z, h, isf = entry[0], entry[1], entry[2], entry[3]
                    zv = entry[4] if len(entry) > 4 else None
                    vm = entry[5] if len(entry) > 5 else None
                    gm, ct, sc0, _ = head.regime_inference(
                        z, h, is_first=isf, z_var=zv, cache_estep=True, valid=vm)
                    head.update_globals(z, h, gm, ct, sc0, is_first=isf, z_var=zv,
                                        batch_id=bid, stats_only=True, valid=vm)
                head.global_step_from_totals()
