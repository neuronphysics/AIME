...
class EMATeacher:
    ...
    def __init__(self, student, tau: float = 0.005, probe=None,
                 drift_tol: float = 0.05, device=None, dynamics=None):
        """
        dynamics  optional RSSM.  When given, the drift probe measures the FULL
                  posterior representation the move buffer stores -- posterior
                  means AND deterministic states -- under an EMA copy of the
                  RSSM's posterior-path parameters (_img_in_layers, _cell,
                  _obs_out_layers, _obs_stat_layer) swapped in at probe time.
                  An encoder-only probe misses changes to the posterior head and
                  GRU, which move the buffer's coordinates just as much.
        """
        """
        ...
        if dynamics is not None:
            self._dyn = dynamics
            self._dyn_ema = {k: v.detach().clone() for k, v in self._posterior_state(dynamics).items()}
    ...
    # ------------------------------------------------------------------ core
    _POSTERIOR_MODULES = ("_img_in_layers", "_cell", "_obs_out_layers", "_obs_stat_layer")

    @staticmethod
    def _posterior_state(dyn):
        out = {}
        for name in EMATeacher._POSTERIOR_MODULES:
            mod = getattr(dyn, name, None)
            if mod is None:
                continue
            for pn, pv in mod.named_parameters():
                out[f"{name}.{pn}"] = pv
            for bn, bv in mod.named_buffers():
                out[f"{name}.{bn}"] = bv
        return out

    @torch.no_grad()
    def _posterior_replay(self, dyn, embed, action, is_first):
        """Posterior means + deter along the probe, mirroring networks.RSSM.obs_step
        (sample=False) but without the SHS prior/filter side effects."""
        B, T = embed.shape[:2]
        state = None
        outs = []
        for t in range(T):
            ...
            if state is None or float(f.sum()) == len(f):
                ...
                state = {k: v * (1.0 - fr.reshape(fr.shape + (1,) * (v.dim() - 2))) +
                         init[k] * fr.reshape(fr.shape + (1,) * (v.dim() - 2)) for k, v in state.items()}
            deter = dyn._deter_step(state, a)
            x = dyn._obs_out_layers(torch.cat([deter, e], -1))
            stats = dyn._suff_stats_layer("obs", x)
            mean = stats["mean"] if "mean" in stats else dyn.get_dist(stats).mode()
            std = stats.get("std")
            if std is None:
                std = torch.zeros_like(mean)
            # Fixed unit noise (self._eps), so the probe is a deterministic
            # function of the parameters while still moving when the posterior
            # SCALE moves: the buffer stores sampled trajectories, so a change
            # in q's std changes the buffer's coordinates even if means are
            # unchanged.  The std itself is also part of the probe vector.
            eps = self._probe_eps(mean.shape, mean.device, mean.dtype, t)
            stoch = mean + std * eps
            state = dict(state); state.update(stats); state["stoch"] = stoch; state["deter"] = deter
            outs.append(torch.cat([stoch, std, deter], -1))
        return torch.stack(outs, 1)

    def _probe_eps(self, shape, device, dtype, t):
        key = (tuple(shape), str(device), str(dtype))
        cache = self.__dict__.setdefault("_eps_cache", {})
        if key not in cache:
            g = torch.Generator(device="cpu").manual_seed(20260908)
            cache[key] = [torch.randn(shape, generator=g).to(device=device, dtype=dtype)
                          for _ in range(256)]
        return cache[key][t % len(cache[key])]

    @torch.no_grad()
    def _encode_raw(self, x):
        out = self.teacher(x)
        emb = (out[0] if isinstance(out, (tuple, list)) else out).detach()
        if self._dyn is None or "action" not in x or "is_first" not in x:
            return emb
        from torch.nn.utils.stateless import _reparametrize_module
        with _reparametrize_module(self._dyn, self._dyn_ema):
            return self._posterior_replay(self._dyn, emb, x["action"], x["is_first"]).detach()
    ...
    def update(self, student):
        ...
        if self._dyn is not None:
            cur = self._posterior_state(self._dyn)
            for k, v in self._dyn_ema.items():
                src = cur.get(k)
                if src is None or src.shape != v.shape:
                    continue
                if src.requires_grad:
                    v.mul_(1.0 - self.tau).add_(src.detach(), alpha=self.tau)
                else:
                    v.copy_(src)
        self._n_updates += 1
    ...
    def state_dict(self):
        return {"teacher": self.teacher.state_dict(), "tau": self.tau,
                ...
                "dyn_ema": (None if self._dyn_ema is None else {k: v.cpu() for k, v in self._dyn_ema.items()})}

    def load_state_dict(self, sd):
        self.teacher.load_state_dict(sd["teacher"])
        self.tau = sd.get("tau", self.tau)
        self.version = sd.get("version", 0)
        if self._dyn_ema is not None and sd.get("dyn_ema") is not None:
            for k, v in sd["dyn_ema"].items():
                if k in self._dyn_ema and self._dyn_ema[k].shape == v.shape:
                    self._dyn_ema[k].copy_(v.to(self._dyn_ema[k].device))
