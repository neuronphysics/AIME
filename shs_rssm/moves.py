...
# to restore the cheaper behaviour when move cost dominates.
...
def _per_seq_occupancy_from_buffer(head, buf):
    ...
    for b in buf.batches:
        gamma, _, _, _ = head.regime_inference(
            ...
            prev_stoch=getattr(b, "prev_stoch", None),
            episode_first=getattr(b, "episode_first", None),
            z_var=b.z_var, action=getattr(b, "action", None), cache_estep=False)
        rows.append(gamma.detach().sum(dim=1).cpu().numpy())
    return np.concatenate(rows, axis=0) if rows else np.zeros((0, head.K))
...
class _Batch:
    ...
    # Genuine episode starts, as distinct from `is_first`, which upstream forces
    # True at every replay crop.  Without it, move scoring counts ~batch_size
    # artificial starts per batch while the M-step counts genuine ones.
    episode_first: torch.Tensor | None = None
    # The SAMPLED trajectory the recognition GRU actually consumed.  When
    # present, the predecessor in g_t is this sample (the one that produced
    # h_t and hence q(z_t | h_t)) and no predecessor variance is added: that is
    # the Rao-Blackwellised estimate under the sequential recognition posterior
    # (shs_moment_consistent).  Absent, the analytic independent-marginals
    # approximation (mean + shifted variance) is used.
    prev_stoch: torch.Tensor | None = None


def _batch_g(head, b):
    """(prev, g, g_var) for a buffered batch under ONE convention: if the batch
    stored the sampled predecessor the recognition GRU consumed, condition on it
    and add no predecessor variance; otherwise use the analytic marginals."""
    ps = getattr(b, "prev_stoch", None)
    prev = head._prev_stoch(b.stoch if ps is None else ps, b.is_first)
    g = head.build_g(prev, b.deter,
                     head._shift_action(getattr(b, "action", None), b.is_first))
    g_var = (None if ps is not None
             else head._g_var_from_z_var(b.z_var, g, is_first=b.is_first))
    return prev, g, g_var


def _batch_masks(head, b, B, T, dtype, device):
    """(valid, ev_mask, hmm_is_first, assume_start_at_t0) for a buffered batch --
    the contract training inference and move acceptance also use."""
    epf = getattr(b, "episode_first", None)
    evm = head._crop_ev_mask(b.is_first, B, T, dtype, device, episode_first=epf)
    isf, sa0 = head._hmm_boundary(b.is_first, epf)
    return getattr(b, "valid", None), evm, isf, sa0


def _batch_evm(head, b):
    """Evidence mask for a buffered batch, in (B, T, 1) form: a fabricated crop
    position contributes no emission statistics, so seed responsibilities are
    zeroed there exactly as the accepted-move accumulator does."""
    B, T = b.stoch.shape[0], b.stoch.shape[1]
    evm = head._crop_ev_mask(b.is_first, B, T, b.stoch.dtype, b.stoch.device,
                             episode_first=getattr(b, "episode_first", None))
    return None if evm is None else evm.reshape(B, T, 1)


def _mask_seed(head, b, resp):
    evm = _batch_evm(head, b)
    return resp if evm is None else resp * evm.to(resp.dtype)


def _batch_g_z_var(head, b):
    return (None if getattr(b, "prev_stoch", None) is not None
            else head._shift_var(b.z_var, b.is_first))
...
class MoveBuffer:
    ...
    def add(self, stoch, deter, is_first=None, z_var=None, step=0,
            ...
            z_cov=None, zg_xcov=None, episode_first=None, prev_stoch=None):
        ...
        entry = _Batch(
            ...
            None if episode_first is None else episode_first.detach())
        ...
        entry.prev_stoch = None if prev_stoch is None else prev_stoch.detach()
...
def _accumulate(head, buffer, regimes, hdp, rstick=None, row_mask=None):
    ...
    for _bi, b in enumerate(buffer.batches):
        ...
        prev, g, g_var = _batch_g(head, b)
        _cond_prev = getattr(b, "prev_stoch", None) is not None
        ev = regimes.expected_loglik(b.stoch, g, z_var=b.z_var, g_var=g_var).double()
        log_init, log_trans, aux = head._score_potentials(
            b.deter, hdp, rstick, action=act, is_first=b.is_first,
            z_mean=b.stoch, z_var=getattr(b, 'z_var', None))
        
        vm, evm, _hmm_isf, _sa0 = _batch_masks(head, b, b.stoch.shape[0], b.stoch.shape[1],
        ...
        gamma, xicount, _, xi = forward_backward(
            log_init, log_trans, ev, is_first=_hmm_isf, valid=vm, ev_mask=evm,
        ...
        # sufficient statistics see no fabricated timestep; start counts and
        # edge counts use the full belief, exactly as bound_local does.
        gamma_st = (gamma if evm is None
                    else gamma * evm.reshape(*gamma.shape[:-1], 1).to(gamma.dtype))
        st = regimes.stats_from_batch(gamma_st, b.stoch, g, z_var=b.z_var,
                                      g_z_var=_batch_g_z_var(head, b),
                                      z_cov=_zc, g_zcov=_gzc, zg_xcov=_xc)
        agg = st if agg is None else {k: agg[k] + st[k] for k in agg}
        Cc += counts.double()
        sc += start_counts_from(gamma, _hmm_isf, valid=vm,
...
def _merge_stats(stats, i, j):
    ...
    # ALL statistics are summed, including the low-rank factor block
    # (Szf/Sfr/Sfh/Sff).  Copying those from the dominant regime -- the previous
    # behaviour -- kept a consistent U basis but discarded N_j/(N_i+N_j) of the
    # factor evidence while N itself was summed, so the candidate was internally
    # inconsistent.  Measured on an exact-duplicate merge (which is lossless by
    # construction) that cost 48 nats over 270 timesteps, in the direction that
    # REJECTS good merges and biases K upward.
    #
    # Summing is exact when the two regimes share a U (the duplicate case) and
    # is a basis-mixing approximation otherwise -- but the candidate is refit
    # before scoring whenever q_rank > 0 (see merge_move), so the sum only has
    # to be a sane initialiser, and unlike the copy it conserves mass.
    for k in s:
        s[k][i] = s[k][i] + s[k][j]
...
def _apply(head, regimes, hdp, C, start, rstick=None, row_map=None, scored_bound=None):
    # The installed C / start / regime statistics are sums over the scoring
    # window (mass M0 = number of buffer batches).  Subsequent EMA updates
    # decay the excess as M_n = 1 + (M0 - 1) * (1 - tau)**n until the mass-one
    # steady state is restored; normalising at install would require rescaling
    # every factor's statistics uniformly (regimes, counts, PG).
    head._last_scored_bound = (None if scored_bound is None else float(scored_bound))
    ...
    if row_map is not None:
        _new = row_map.detach().to(torch.float32).cpu()
    elif regimes.K >= K_old:
        _new = torch.zeros(regimes.K, K_old, dtype=torch.float32)
        _new[:K_old, :K_old] = torch.eye(K_old)
    else:
        _new = None
    # Beliefs held by live rollout states were formed BEFORE any move in this
    # sweep, so successive maps must compose: K_new x K_old_i @ K_old_i x K_orig.
    _prev = getattr(head, "_belief_map", None)
    if _new is None:
        head._belief_map = None
    elif _prev is not None and int(_prev.shape[0]) == int(_new.shape[1]):
        head._belief_map = _new @ _prev
    else:
        head._belief_map = _new
        ...
        if mode == "frozen":
            ...
            if _accept(cand, base, head.K - 1, head.K, threshold, size_log_prior_odds):
                ...
                _apply(head, regimes, hdp, C, start, rstick=cand_rs,
                       row_map=_rowmap_keep(head.K, keep, device=soft.device), scored_bound=cand)
...
def merge_move(head, stoch=None, deter=None, is_first=None, *, buffer=None,
    ...
    for _ in range(max(1, int(max_passes))):
        ...
        for (i, j, _) in pairs:
            ...
            _needs_refit = (cand_rs is not None and _MERGE_REFIT_RECURRENT) or \
                           int(getattr(head.regimes, "q_rank", 0)) > 0
            if not bool(_MERGE_REFIT) and not _needs_refit:
                pass
            else:
                ...
                _it = (max(1, int(refine_iters)) if (cand_rs is not None
                       or int(getattr(head.regimes, "q_rank", 0)) > 0)
                       else int(refine_iters))
                regimes, hdp, C, start = _refine(head, regimes, hdp, buf,
            ...
            if _accept(cand, base, K - 1, K, threshold, size_log_prior_odds):
                _apply(head, regimes, hdp, C, start, rstick=cand_rs,
                       row_map=_rowmap_keep(K, keep, sum_into=(i, j),
                                            device=keep.device), scored_bound=cand)
                any_accept = True
...
def _best_cut_marginal(z, g, w, regimes, min_block=3):
    ...
    if W < 2 * min_block:
        return max(1, W // 2)
    # Shared-carry heads model z = A r + C htil + noise with r = [z_prev, a, 1].
    # Score the cut on the carry residual z - C htil and on the r coordinates
    # (log_marginal_from_stats selects those coordinates).
    if hasattr(regimes, "Cmean") and hasattr(regimes, "Hp") and getattr(regimes, "Hp", 0) > 0:
        htil = g[:, regimes.L: regimes.L + regimes.Hp]
        z = z - htil @ regimes.Cmean.to(z.dtype).T
    wz = (w[:, None] * z)
...
def seqcreate_birth_move(head, stoch=None, deter=None, is_first=None, *, buffer=None,
    ...
    for n in rng.permutation(len(idx_map))[:max(1, int(n_proposals))]:
        ...
        t_agg = head.regimes.stats_from_batch(
            _mask_seed(head, b, seedr), b.stoch, gw, z_var=b.z_var,
            g_z_var=_batch_g_z_var(head, b),
            z_cov=_zc, g_zcov=_gzc, zg_xcov=_xc)
        ...
        for _ in range(int(n_refine)):
            t_agg, t_C, t_s, t_pg = _accumulate(head, buf, regimes, hdp,
                                                rstick=cand_rs, row_mask=only)
            if (getattr(head, 'recurrent', False) and cand_rs is not None
                    and t_pg is not None):
                cand_rs.pg_update_statewise(t_pg["phi"], t_pg["r_mass"],
                                            t_pg["row_weight"], lr=None)
            agg = {k: fr_agg[k] + t_agg[k] for k in fr_agg}
        ...
        if _accept(cand, base, K + 2, K, threshold, size_log_prior_odds):
            _apply(head, regimes, hdp, fr_C + t_C, fr_s + t_s, rstick=cand_rs,
                   row_map=_rowmap_append(K, 2, device=head.z0.device), scored_bound=cand)
            any_accept = True
            total_gain += cand - base
    return any_accept, total_gain
...
def interval_birth_move(head, stoch=None, deter=None, is_first=None, *, buffer=None,
    ...
    for bi, b in enumerate(buf.batches):
        ...
        _, g, _ = _batch_g(head, b)
        _zc, _gzc, _xc = _smoothed_moments(head, b)
        st = head.regimes.stats_from_batch(_mask_seed(head, b, new_resp), b.stoch, g, z_var=b.z_var,
                                           g_z_var=_batch_g_z_var(head, b),
                                           z_cov=_zc, g_zcov=_gzc, zg_xcov=_xc)
        agg = st if agg is None else {k: agg[k] + st[k] for k in agg}
...
def birth_move(head, stoch=None, deter=None, is_first=None, *, buffer=None,
    ...
    for b in buf.batches:
        ...
        vm, _evm_b, _hisf_b, _sa0_b = _batch_masks(head, b, b.stoch.shape[0], b.stoch.shape[1],
                                                   b.stoch.dtype, b.stoch.device)
        gamma, _, _ = forward_backward(log_init, log_trans, ev_s, is_first=_hisf_b,
                                       valid=vm, ev_mask=_evm_b,
                                       assume_start_at_t0=_sa0_b)
        scale = best.std() + 1e-6
    ...
    if _accept(cand, base, K + 1, K, threshold, size_log_prior_odds):
        _apply(head, regimes, hdp, Cc, sc, rstick=cand_rs,
               row_map=_rowmap_append(K, 1, device=head.z0.device), scored_bound=cand)
        return True, cand - base
    return False, cand - base
...
def split_move(head, stoch=None, deter=None, is_first=None, *, buffer=None,
    ...
    for kpar in torch.argsort(soft, descending=True).tolist()[:confirm_top]:
        ...
        for b in buf.batches:
            ...
            _zc, _gzc, _xc = _smoothed_moments(head, b)
            st = head.regimes.stats_from_batch(_mask_seed(head, b, new), b.stoch, g, z_var=b.z_var,
                                               g_z_var=_batch_g_z_var(head, b),
                                               z_cov=_zc, g_zcov=_gzc, zg_xcov=_xc)
            agg = st if agg is None else {k: agg[k] + st[k] for k in agg}
...
def sweep_moves(head, stoch=None, deter=None, is_first=None, *, buffer=None,
    ...
    def _verify(tag, res):
        if _chk and isinstance(res, tuple) and res[0]:
            scored = getattr(head, "_last_scored_bound", None)
            if scored is None:
                return res     
            verify_bound_tracking(head, buf, scored, tag=tag,
