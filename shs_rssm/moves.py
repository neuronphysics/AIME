...
class MoveBuffer:
    ...
    def add(self, stoch, deter, is_first=None, z_var=None, step=0,
            ...
            z_cov=None, zg_xcov=None, episode_first=None):
...
def _accumulate(head, buffer, regimes, hdp, rstick=None, row_mask=None):
    ...
    for _bi, b in enumerate(buffer.batches):
        ...
        g = head.build_g(prev, b.deter, head._shift_action(act, b.is_first))
        ...
        Cc += counts.double()
        _bepf = getattr(b, "episode_first", None)
        sc += start_counts_from(
            gamma, _bepf if _bepf is not None else b.is_first, valid=vm,
            assume_start_at_t0=(False if _bepf is not None else _sa0)).double()
    pg = None
    if pg_phi:
