...
def attend(q, k, v, dropout_prob=0.0, attention_mask=None):
    ...
    if attention_mask is not None:
        large_value = 1e4 if attention.dtype == torch.float16 else 1e30
        attention = torch.where(
            attention_mask.unsqueeze(-3),  # Adding a dimension for heads
            attention,
            torch.full_like(attention, -large_value)
        )
...
class Attention(nn.Module):
    """Multi-headed {cross, self}-attention in PyTorch."""
    ...
    def forward(self, inputs_q, inputs_kv, attention_mask=None):
        # Linear projections, we already applied layer norm before linear
