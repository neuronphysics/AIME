...
import matplotlib
matplotlib.use("Agg")
def compute_param_cosines(named_params, task_losses, patterns=None, return_norms: bool = False):
    """
    Build unit gradient directions for each task w.r.t. a selected parameter set
    using loss.backward() instead of torch.autograd.grad (works with FairScale
    activation checkpointing).

    Returns:
        G: [T, D] (unit vectors on CPU)
        cos: [T, T] cosine matrix
        norms (optional): [T] gradient L2 norms (on CPU)
    """
    ...
    # Extract only tensor losses
    tensor_losses = [obj for obj in task_losses if torch.is_tensor(obj)]
    if len(tensor_losses) == 0:
        raise RuntimeError("No task loss produced grads for selected parameters.")

    # Save existing grads so we can restore them afterwards
    saved_grads = [p.grad.clone() if p.grad is not None else None for p in params]

    grads = []
    norms = []
    T = len(tensor_losses)

    for idx, obj in enumerate(tensor_losses):
        # Zero current grads for tracked params
        for p in params:
        ...
        v = torch.cat(flat)
        n = v.norm()
        norms.append(n.detach().cpu())
        grads.append(v / (n + 1e-12) if float(n) > 0 else torch.zeros_like(v))

    # Restore original grads (so diagnostics don't interfere with training)
...
def compute_shared_cosines(shared_repr, task_losses, return_norms: bool = False):
    """
    Unit grads and cosines w.r.t. a shared representation tensor z,
    using loss.backward() instead of torch.autograd.grad.
    """
    z = shared_repr
    if not z.requires_grad:
        raise RuntimeError("shared_repr must require grad")

    tensor_losses = [obj for obj in task_losses if torch.is_tensor(obj)]
    if len(tensor_losses) == 0:
        raise RuntimeError("No task loss depends on shared_repr (z).")

    saved_grad = z.grad.clone() if z.grad is not None else None
...
class GradDiagnosticsAggregator:
    ...
    def update(self, model, shared_repr, task_losses, named_params=None, param_patterns=None):
        """
        If shared_repr is not None: compute directions and amplitudes w.r.t. z.
        If shared_repr is None: compute them w.r.t. selected parameters.
        """
        if shared_repr is not None:
            # ∇_z path
            assert shared_repr.requires_grad, "shared_repr must require grad during eval diagnostics"
            G, cos, norms = compute_shared_cosines(shared_repr, task_losses, return_norms=True)
            if self.sum_unit_grads is None:
                self.sum_unit_grads = torch.zeros_like(G)
            self.sum_unit_grads += G.detach()
            self.sum_cos += cos.detach()
            self.count += 1
            # accumulate RMS amplitude
            self._sum_sq_shared += norms**2
        else:
            # parameter-based path
            if named_params is None:
                named_params = list(model.named_parameters())
            G, cos, norms = compute_param_cosines(named_params, task_losses,
                                                patterns=param_patterns, return_norms=True)
            if self.sum_unit_grads is None:
                self.sum_unit_grads = torch.zeros_like(G)
            self.sum_unit_grads += G.detach()
            self.sum_cos += cos.detach()
        ...
        # Optional per-component norms (this already uses loss.backward, so it's fine)
