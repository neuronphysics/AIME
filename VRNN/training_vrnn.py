...
def run_train(modelstate, loader_train, loader_valid, device, dataframe, path_general, file_name_general, lr,
              max_epochs, train_rank, train_sampler, batch_size):
    ...
    def train(epoch):
        ...
        for i, (u, y) in enumerate(loader_train):
            ...
            if torch.cuda.is_available():
                ...
                grad_params = [
                    p * inv_scale if p is not None and not torch.isnan(p).any() else torch.tensor(0, device=device,
                                                                                                  dtype=torch.float32)
                    for p in scaled_grad_params]
                ...
                torch.nn.utils.clip_grad_norm_(modelstate.model.parameters(), max_norm=train_options['clip'],
            ...
            if i % train_options['print_every'] == 0:
                print(
                    'Train Epoch: [{:5d}/{:5d}], Batch [{:6d}/{:6d} ({:3.0f}%)]\tLearning rate: {:.2e}\tTotal Loss: {:.3f}\t Discriminator Loss: {:.4f}'.format(
                        ...
                        discriminator_loss.item()))  # total_batches
    ...
    try:
        ...
        progress_bar = tqdm(initial=0, leave=True, total=train_options['n_epochs'], desc=process_desc.format(0, 0, 0),
        ...
        for epoch in range(0, train_options['n_epochs'] + 1):
            ...
            if epoch % train_options['test_every'] == 0:
                ...
                print('Train Epoch: [{:5d}/{:5d}], Batch [{:6d}/{:6d} ({:3.0f}%)]\tLearning rate: {:.2e}\tLoss: {:.3f}'
                      '\tVal Loss: {:.3f}'.format(epoch, train_options['n_epochs'], len(loader_train),
                                                  len(loader_train),
                                                  100., lr, loss, vloss))
...
def main(arg):
    ...
    train_x = hard_chunk_data(x_train, max_seq)
    train_y = hard_chunk_data(y_train, max_seq)
    test_x = hard_chunk_data(x_test, max_seq)
    test_y = hard_chunk_data(y_test, max_seq)
    validation_x = hard_chunk_data(x_val, max_seq)
    validation_y = hard_chunk_data(y_val, max_seq)
...
def hard_chunk_data(data, max_seq):
    return data[:, :, :max_seq]
