...
def _calculate_n_step_returns( rewards:torch.Tensor,
                               ...
                               lambd: float= 0.95, # The GAE (lambda) parameter:Factor for trade-off of bias vs variance for Generalized Advantage Estimator.
    ...
    n_step_returns = torch. torch.zeros(rewards.shape, dtype=torch.float32).to(device=device)
    
    gae=torch. torch.zeros(batch_size, dtype=torch.float32).to(device=device)
    for t in reversed(range(n_step)):
        masked =  (1.0 - done[:,t+1].float().to(device=device))
...
class D2EAgent(Agent):
  ...
  def _build_q_loss(self, batch):
    ...
    q1_target = _calculate_n_step_returns( r, target_v_next, real_v_target, done,  self._discount, self._gae_lambda, self.device)
  ...
  def _build_v_loss(self, batch):
    ...
    v_target = q1_pred_ensemble - self._get_alpha_entropy()[0] * log_a_pi - self._get_alpha()[0] * (div_estimate + disc_estimate_loss)
  ...
  def _optimize_step(self, batch):
    ...
    if self._global_step%10 == 0:
       logging.info("Policy Loss:{}, Value Loss:{}, Q Loss: {}, Reward: {}, q1_target:{}, q2_target:{}, transition dynamics gan:{} ".format(info["policy_loss"], info["value_loss"], info["Q_loss"], info["reward_mean"], info["q1_target_mean"], info["q2_target_mean"], info["transition_fgan"]))
    return info
