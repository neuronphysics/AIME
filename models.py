...
class WorldModel(nn.Module):
    ...
    def _train(self, data):
        ...
        with tools.RequiresGrad(self):
            with torch.cuda.amp.autocast(self._use_amp):
                embed = self.encoder(data)
                 # `is_first` stays the sequence boundary (upstream semantics);
                # `episode_first` carries genuine episode starts for the HDP.
                _obs_kw = {}
                if isinstance(self.dynamics, SHSRSSM) and "episode_first" in data:
                    _obs_kw["episode_first"] = data["episode_first"]
                post, prior = self.dynamics.observe(
                    embed, data["action"], data["is_first"], **_obs_kw
                )
        ...
        # ---- slow teacher reference for move scoring -----------------------
        # A fixed probe batch measures encoder drift in OUTPUT space; the SHS
        # sweep gate refuses structural moves while drift exceeds tolerance, so
        # a merge/birth is never scored across two coordinate systems.
        _tc = getattr(self.dynamics, "_shs_teacher_cfg", None)
        if _tc is not None and _tc.get("enabled") and getattr(self.dynamics, "_teacher", None) is None:
            try:
                from shs_rssm.teacher import EMATeacher
                _probe = {k: v[:2].detach().clone() for k, v in data.items()
                          if torch.is_tensor(v)}
                self.dynamics._teacher = EMATeacher(
                    self.encoder, tau=_tc["tau"], probe=_probe,
                    drift_tol=_tc["drift_tol"], dynamics=self.dynamics)
                print(f"[SHS] teacher reference active (tau={_tc['tau']}, "
                      f"drift_tol={_tc['drift_tol']})")
            except Exception as _e:
                print(f"[SHS] teacher disabled ({type(_e).__name__}: {_e})")
                _tc["enabled"] = False
                self.dynamics._teacher = None
        # repr_version stamps every move-buffer batch with the coordinate system
        # it was encoded in; _live_sweep_allowed() purges batches whose stamp
        # differs from the current version and needs >= 4 survivors.  Bumping on
        # EVERY gradient step (the previous behaviour) left at most one batch
        # alive at any scheduled sweep, so live merge/delete/birth never ran in
        # RL training and K stayed at its initial value.  Bump only when the
        # teacher probe reports a material drift event (its own version counter),
        # which is what the stamp was meant to express; without a teacher,
        # shs_move_max_age bounds staleness in time instead.
        _t = getattr(self.dynamics, "_teacher", None)
        if _t is not None:
            try:
                _t.update(self.encoder)
            except Exception as _e:
                print(f"[SHS] teacher update failed, disabling ({_e})")
                self.dynamics._teacher = None
                _t = None
        if _t is not None and hasattr(self.dynamics, "bump_repr_version"):
            _tv = int(getattr(_t, "version", 0))
            if _tv != getattr(self, "_shs_last_teacher_version", 0):
                self._shs_last_teacher_version = _tv
    ...
    def shs_diagnostics(self, data, step, logdir, context=5, make_figures=True):
        ...
        _rs = getattr(self.dynamics.regime, "rstick", None)
        if _rs is not None and getattr(_rs, "hier_bias", False):
            _rep = _rs.hier_report()
            metrics["shs_gate_hier_logit_mean"] = _rep["logit_mean"]
            metrics["shs_gate_hier_eff_var"] = _rep["eff_var"]           # 1/E[lam], used in the update
            metrics["shs_gate_hier_pred_var"] = _rep["pred_var"]         # E[1/lam] + Var[mu], new-row predictive
            metrics["shs_gate_hier_persist_plugin"] = _rep["persist_plugin"]     # sigmoid(E[mu])
            metrics["shs_gate_hier_persist_expected"] = _rep["persist_expected"] # ~E[sigmoid(r)], new row
...
class ImagBehavior(nn.Module):
    ...
    def _compute_actor_loss(
        ...
        if self._config.reward_EMA:
            ...
            # RewardEMA returns clip(P95 - P05, min=1) -- exactly max(1, S) from
            # DreamerV3 eq. 7.  The REINFORCE branch below divides by it.
            _ret_scale = scale
        else:
            adv = target - base
            _ret_scale = 1.0
        ...
        if self._config.imag_gradient == "dynamics":
            ...
            # REINFORCE actor gradient, DreamerV3 eq. 6:
            #   sg( (R^lambda - v(s)) / max(1, S) ) * log pi(a|s)
            # The sampled action is detached (score-function estimator) and
            # the advantage is normalised by RewardEMA's max(1, S).  The
            # offset term is absorbed by the value baseline and omitted.
            # Note: dreamerv3-torch uses the raw advantage here, so reinforce
            # runs are not numerically comparable with that repo.
            actor_target = (
                policy.log_prob(imag_action.detach())[:-1][:, :, None]
                * ((target - self.value(imag_feat[:-1]).mode()) / _ret_scale).detach()
            )
