...
try:
    ...
    import Image
from infGaussian_utilities import AverageMeter, ResidualBlock, LinearResidual, ResidualBlock_deconv, UnetEncoder, DecoderCell, DoubleConv
    ...
    def __init__(self,nchannel, z_dim, hidden_dim, img_width, max_filters=512, num_layers=4, small_conv=False):
        ...
        if img_width == 64:
            self.encoder = nn.Sequential(
                nn.Conv2d(self.nchannel, self.dim_h, 5, 2, 1, bias=False),
                nn.ReLU(False),
                ...
                nn.ReLU(False),
                nn.Flatten(),
                nn.Linear(self.dim_h * (2 ** 4), hidden_dim, bias=True),
            ...
            self.encoder = nn.Sequential(
                ...
                nn.ReLU(False),
                nn.Flatten(),
                nn.Linear(self.dim_h * (2 ** 3), hidden_dim, bias=True),
                nn.BatchNorm1d(hidden_dim),
                nn.ReLU(False),
            ) 
        else:
            assert False, "doesn't support img_width " + str(img_width)
        ...
        #self.fc_mu         = nn.Linear(hidden_dim, z_dim, bias=True)
        
        # variance of z

        self.fc_log_var    = nn.Linear(hidden_dim, z_dim, bias=False)
        #self.fc_log_var    = nn.Linear(hidden_dim, z_dim, bias=True)
        self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        self.to(device=self.device)
    ...
    def __init__(self,nchannel, z_dim, hidden_dim, extend_dim, img_width, max_filters=512, num_layers=4, small_conv=False):
        ...
        if img_width == 64:
            self.decoder = nn.Sequential(
                nn.Linear(z_dim, self.dim_h * 16 * 4 * 4),
                nn.ReLU(False),
                nn.Unflatten(1, (self.dim_h * 16, 4, 4)),
                ...
                nn.BatchNorm2d(self.dim_h * 2),
                nn.ReLU(False),
                nn.ConvTranspose2d(self.dim_h * 2, self.nchannel, 4, 2, 1),
                nn.Sigmoid()
            )
        elif img_width == 32:
            self.decoder = nn.Sequential(
                nn.Linear(z_dim, self.dim_h * 8 * 4 * 4),
                nn.ReLU(False),
                nn.Unflatten(1, (self.dim_h * 8, 4, 4)),
                nn.ConvTranspose2d(self.dim_h * 8, self.dim_h * 4, 4, 2, 1),
                nn.BatchNorm2d(self.dim_h * 4),
                nn.ReLU(False),
                nn.ConvTranspose2d(self.dim_h * 4, self.dim_h * 2, 4, 2, 1),
...
class GMMVAE(nn.Module):
    ...
    def __init__(self, number_of_mixtures, nchannel, z_dim, w_dim, hidden_dim,  device, img_width, batch_size, max_filters=512, num_layers=4, small_conv=False ,dropout=0.1):
        ...
        cfg = {'PicaNet': "GGGLLL",
            ...
            'Channel': [w_dim, 256, 128, 96, 64, 32, 16],
...
class InfGaussMMVAE(GMMVAE,BetaSample):
    ...
    def get_ELBO(self, X):
        ...
        #elbo5     = criterion(self.x_recons, self.X)
    ...
    def get_component_samples(self, batchSize):
        ...
        #print(torch.ones((batchSize,self.K-1)).shape, self.prior_nu.shape)
