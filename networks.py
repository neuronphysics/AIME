    ...
    def observe(self, embed, action, is_first, state=None, sample=True):
        ...
        post, prior = tools.static_scan(
            lambda prev_state, prev_act, embed, is_first: self.obs_step(
                prev_state[0], prev_act, embed, is_first, sample=sample
            ),
...
class MultiDecoder(nn.Module):
    def __init__(
        ...
        shapes = {k: v for k, v in shapes.items()
                  if k not in excluded and not k.startswith("factor_")}
        self.cnn_shapes = {
            k: v for k, v in shapes.items() if len(v) == 3 and re.match(cnn_keys, k)
        }
        self.mlp_shapes = {
...
class MLP(nn.Module):
    def __init__(
        ...
        act = getattr(torch.nn, act)
        self._dist = dist
        self._std = std if isinstance(std, str) else torch.tensor((std,), device=device or "cpu")
        self._min_std = min_std
