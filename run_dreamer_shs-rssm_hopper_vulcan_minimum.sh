...
#SBATCH --output=/home/memole/scratch/AIME/logs/dreamerv3-shs-rssm-hopper_%A_%a.out
#SBATCH --error=/home/memole/scratch/AIME/logs/dreamerv3-shs-rssm-hopper_%A_%a.err
...
SHS_ACTION_DIM="${SHS_ACTION_DIM:-4}"      # hopper has 4 actuators
FLAGS="${FLAGS:-}"

LOGDIR="${LOGDIR:-${PROJECT_DIR}/logdir/dmc_hopper_shs_seed_${SEED}}"
...
echo "Hopper:    seed=${SEED} steps=${STEPS} batch=${BATCH_SIZE}x${BATCH_LENGTH}"
echo "           q_rank=${SHS_Q_RANK} action_dim=${SHS_ACTION_DIM} (hopper has 4 actuators)"
echo "           logdir=${LOGDIR}"
echo "           FLAGS='${FLAGS}'"
...
# ---- GL preflight: try EGL, fall back to OSMesa, fail fast otherwise ------
unset DISPLAY
render_ok=0
if python - <<'PY'
from dm_control import suite
suite.load("hopper", "hop").physics.render(64, 64)
...
python -u dreamer.py --configs dmc_vision dmc_hopper_shs \
  --seed "${SEED}" \
  --compile False \
