...
- **Policy:** Latent-space **Adversarially Guided Actor-Critic (AGAC)** insired behavior:
  ...
  - Old-policy options:
    ...
    - `snapshot` – strict PPO-style snapshot once per batch.
- Logging:
  - TensorBoard summaries.
  - Optional **Weights & Biases (WandB)** logging for scalars, images, and videos.  
...
```
python3 -m dreamerv3.dreamer --configs dmc_vision agac_recursive --task dmc_walker_walk --logdir ./runs/logdir/dmc_walker_walk
```
Monitor results:
```
