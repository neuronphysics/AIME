...
class Logger:
    ...
    def write(self, fps=False, step=False):
        ...
        scalar_dict = {k: float(v) for k, v in scalars} 
        ...
        if self._wandb_run is not None:
            ...
            # scalars
            wandb_log.update(scalar_dict)

            # images
            for name, value in self._images.items():
                wandb_log[name] = wandb.Image(value)

            # videos: (T, C, H, W) for wandb.Video
            for name, value in self._videos.items():
                name = name if isinstance(name, str) else name.decode("utf-8")
                v = np.array(value)
                if np.issubdtype(v.dtype, np.floating):
                    v = np.clip(255 * v, 0, 255).astype(np.uint8)
                B, T, H, W, C = v.shape
                # Same mosaic as TB, but drop batch dim:
                v = v.transpose(1, 4, 2, 0, 3).reshape((T, C, H, B * W))
                wandb_log[name] = wandb.Video(v, fps=16, format="mp4")
