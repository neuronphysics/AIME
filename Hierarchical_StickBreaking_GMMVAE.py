...
class GMMVAE(nn.Module):
    ...
    def __init__(self, number_of_mixtures, nchannel, z_dim, w_dim, hidden_dim, device, img_width, batch_size,
                 max_filters=512, num_layers=4, small_conv=False, use_mse_loss=False):
        ...
        self.decoder = VAEDecoder(nchannel, z_dim, hidden_dim, self.encoder.h_dim, self.encoder.h_image_dim, img_width,
                                  max_filters, num_layers, small_conv)
        ########################
...
class InfGaussMMVAE(GMMVAE, BetaSample):
    ...
    def get_ELBO(self, X):
        ...
        elbo3 = -0.5 * torch.sum(1 + self.w_x_logvar - self.w_x_mean.pow(2) - self.w_x_logvar.exp()).to(
            device=self.device)  # VAE_KLDCriterion

        # 4)compute E_{p(w|x)p(c|x)}[D_KL(Q(z|x)||p(z|c,w))]

        elbo4 = self.ExpectedKLDivergence(self.c_posterior, self.z_x_mean, self.z_x_logvar, self.z_wc_mean_list_sample,
                                          self.z_wc_logvar_list_sample)
        ...
        elbo5 = criterion(self.x_recons.view(-1, self.nchannel * self.img_size * self.img_size),
                          self.X.view(-1, self.nchannel * self.img_size * self.img_size))
    ...
    def get_log_margLL(self, batchSize):
        ...
        for k in range(self.K):
            all_log_gauss_priors.append(
                log_normal_pdf(self.z_x, self.z_wc_mean_list_sample[k], self.z_wc_logvar_list_sample[k]))
        ...
        log_w_prior = log_normal_pdf(self.w_x, torch.zeros(self.w_x.size()).to(device=self.device),
                                     torch.eye(self.w_x.size()[0], self.w_x.size()[1], device=self.device))
...
def train(epoch):
    ...
    for batch_idx, (X, classes) in enumerate(train_loader):
        ...
        for _ in range(hyperParams["CRITIC_ITERATIONS"]):
            X_recons_linear, mu_z, logvar_z, mu_w, logvae_w, qc, kumar_a, kumar_b, mu_pz, logvar_pz, gamma_alpha, gamma_beta = net(
                X)
    ...
    return train_loss_avg, X_recons_linear, loss_dict['kumar2beta_kld'].item(), loss_dict["wasserstein_loss"].item(), \
           loss_dict['z_latent_space_kld'].item()
...
if __name__ == "__main__":
    ...
    test_dataset = datasets.STL10('./data', split='test', transform=transform,
                                  target_transform=None, download=True)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=hyperParams["batch_size"], shuffle=True, num_workers=1)
