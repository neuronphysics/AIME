...
class Dreamer(nn.Module):
    ...
    def __call__(self, obs, reset, state=None, training=True):
        ...
        # A structural move can happen inside the training loop above, changing
        # the regime identities underneath the acting state that is carried
        # across calls.  Reconcile here -- after all training and consolidation,
        # immediately before the state is used to act and written back.
        _sync = getattr(self._wm.dynamics, "sync_acting_state", None)
        if _sync is not None and isinstance(state, tuple) and len(state) == 2 \
                and isinstance(state[0], dict):
            state = (_sync(state[0]), state[1])
...
def _shs_adapt_state_shapes(agent, sd):
    """Resize K-shaped SHS buffers to the checkpoint's shapes before a strict
    load, so runs that accepted structural moves (K growth/shrink) resume.
    All K-shaped state is registered buffers (no optimizer entries), so this
    is shape surgery only; values are then overwritten by load_state_dict."""
    resized = []
...
def main(config):
    ...
    if (logdir / "latest.pt").exists():
        ...
        _NEW_KEY_SUFFIXES = ("z0_logvar", "b0_c0", "b0_d0", "b0_chat", "b0_dhat")
        ...
        if _missing:
            ...
            warnings.warn(
                f"legacy checkpoint: backfilled {sorted(_missing)} from "
                "initialisation (post-upgrade keys absent from this checkpoint; "
                "initial values reproduce the pre-upgrade behaviour exactly)")
