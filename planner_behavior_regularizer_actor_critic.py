...
class ActorNetwork(nn.Module):
    ...
    def _get_outputs(self, state):
        ...
        """
        a_distribution = TransformedDistribution(
            MultivariateNormal(torch.full_like(mean, 0).to(device=self.device),
...
class BRACAgent(Agent):
    ...
    def _build_optimizers(self):
        ...
        self._q_optimizer = torch.optim.Adam(self._get_q_vars(), lr=opts[0][0], betas=(opts[0][1], opts[0][2]),
                                             weight_decay=self._weight_decays[0])
