...
#SBATCH --error=/home/memole/scratch/AIME/logs/dreamer_shs_finger_turn_hard_%A_%a.err
...
LOGDIR="${LOGDIR:-${PROJECT_DIR}/logdir/dmc_finger_turn_hard_shs_v2_seed_${SEED}}"
...
echo "Finger: seed=${SEED}  steps=${STEPS}  batch=${BATCH_SIZE}x${BATCH_LENGTH}"
echo "Log:    ${LOGDIR}"
...
suite.load("finger", "turn_hard").physics.render(64, 64)
...
# ---- run ------------------------------------------------------------------
python -u dreamer.py --configs dmc_vision dmc_finger_turn_hard_shs_gate \
  --shs_action_dim 2 \
  --shs_b0 0.2 --shs_calibrate_sF 0.1 \
  --shs_K 16 --shs_q_rank 4 --shs_ema_tau 0.01 \
  --seed "${SEED}" \
  --compile False \
