  ...
  with torch.no_grad():
    ...
    for t in pbar:
      with gpytorch.settings.num_likelihood_samples(args.num_gp_likelihood_samples):
        action, _, _, _, _, _, imagined_reward = actor_critic_planner.act(episode_states[-args.lagging_size:], episode_actions[-args.lagging_size:], transition_model, device=args.device, softplus=args.imaginary_rollout_softplus)
...
def rgp_step(n_iter, losses):
    global transition_model, transition_prev_params
    if (not args.use_regular_vae):
      infinite_vae.batch_size = args.batch_size * args.chunk_size
    gp_pbar = tqdm(range(n_iter))
    for s in gp_pbar:
      with gpytorch.settings.num_likelihood_samples(args.num_gp_likelihood_samples): # to do: make this a hyperparameter as well
        ...
        transition_data = TransitionDataset(transition_input.detach(), true_latent.detach())
        transition_prev_params = transition_prev_params[:-args.transition_retain_params]
        transition_imagine_loss = 0
        for t in range(transition_input.size(0)):
            transition_data.filter_by_idx(t)
            transition_model, transition_loss = train(transition_data, None, None, batch_size=1, n_f=transition_input.size(2),
                                                      prev_params=transition_prev_params, device=args.device)
            transition_prev_params.append(transition_model.state_dict())
            transition_imagine_loss += transition_loss
        transition_model.eval()
        transition_imagine_loss /= transition_input.size(0)
        ...
        if args.use_regular_vae:
          ...
          print("observation_loss", observation_loss, "latent_kl_loss", latent_kl_loss)
          losses.append([observation_loss.item(), reward_loss.item(), latent_kl_loss.item(), transition_imagine_loss, controller_imagine_loss.item() ])
        ...
        nn.utils.clip_grad_norm_(param_list, args.grad_clip_norm, norm_type=2)
        optimiser.step()
        del true_latent, true_action, predicted_rewards, predicted_action,  init_states, latent_states, init_actions, actions, observations
    return losses
  ...
  with gpytorch.settings.num_likelihood_samples(args.num_gp_likelihood_samples):
    for t in pbar:
      (
        ...
        transition_dist,
        imagined_reward
      ) = actor_critic_planner.act(episode_states[-args.lagging_size:], episode_actions[-args.lagging_size:], transition_model, device=args.device, softplus=args.imaginary_rollout_softplus)
